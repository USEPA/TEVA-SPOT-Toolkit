...
if (pStrCout) {
   if (!(pStrCout->good()))
      pStrCout->clear();
#ifndef UTILIB_HAVE_SSTREAM
   else if (pStrCout->pcount() > 0)
#else
   else if (pStrCout->str().size() > 0)
#endif
...
if (pStrCerr) {
   if (!(pStrCerr->good()))
      pStrCerr->clear();
#ifndef UTILIB_HAVE_SSTREAM
   else if (pStrCerr->pcount() > 0)
#else
   else if (pStrCerr->str().size() > 0)
#endif
...
#else
int utilib::CommonIO::numDigits = 0;

void CommonIO::dummy_func() {}
