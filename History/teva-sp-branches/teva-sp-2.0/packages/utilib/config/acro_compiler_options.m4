...
if test X$1 != Xsubpackage; then
  ...
  [AC_HELP_STRING([--with-cppflags],[set preprocessor flags (e.g., "-Dflag -Idir")])],
  [USERDEF_CPPFLAGS=${withval}]
  )
  
  AC_ARG_WITH(fflags,
  [AC_HELP_STRING([--with-fflags],[set Fortran compiler flags])],
  [USERDEF_FFLAGS=${withval}]
  )
  
  AC_ARG_WITH(cflags,
  [AC_HELP_STRING([--with-cflags],[set C compiler flags])],
  [USERDEF_CFLAGS=${withval}]
  )
  
  AC_ARG_WITH(cxxflags,
  [AC_HELP_STRING([--with-cxxflags],[set C++ compiler flags])],
  ...
  [AC_HELP_STRING([--with-debugging],[Build with debugging information])],
  ...
  [AC_HELP_STRING([--with-optimization=flag],[Build with the specified optimization flag (Default ${optimization_default}).])],
  [WITH_OPT=$withval],
  [WITH_OPT=notset]
  )
