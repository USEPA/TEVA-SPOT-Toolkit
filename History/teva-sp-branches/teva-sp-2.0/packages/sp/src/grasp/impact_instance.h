...
class PMImpactInstance : public PMInstance 
{
 ...
 protected:
  ...
  std::vector<double> myUndetectedImpactsCompiled;
  std::vector<std::vector<double> > myImpacts;

  #endif

  // the arrival sequences for each attack - not needed for
  ...
  // order of arrival time. the node ids are 1-based.
 ...
 public:
  ...
  double undetectedImpact(int c)
    {
      return myUndetectedImpacts[c];
    }
  ...
  inline double fastGetDist(int c, int f)const
    {
      return myImpacts[c][f];
    }

  inline double fastGetUndetectedImpact(int c)const
    {
      return myUndetectedImpactsCompiled[c];
    }

  inline double fastGetArrivalTime(int c, int f)const
    {
  ...
  bool canDetect(int customer,int facility)
    {
      return myDetectedImpacts[customer].find(facility)!=myDetectedImpacts[customer].end();
    }
