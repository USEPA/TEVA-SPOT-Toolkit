...
sub exec_configure {
  ...
  if ($logfile eq "") {
     ...
     if (@config_options) {
        $status = 0xffff & system("(which automake; which autoconf; autoreconf --install ; ./configure $siteconfig @config_options) > ${outdir}/config.out 2>&1");
     } else {
        $status = 0xffff & system("(which automake; which autoconf; autoreconf --install ; ./configure $siteconfig) > ${outdir}/config.out 2>&1");
     }
...
sub exec_build {
  ...
  if ($logfile eq "") {
     ...
     print LOGFILE "<hr><pre>\n";
     close LOGFILE;
     $status = 0xffff & system("($clean make ) >> ${outdir}/build.out 2>&1");
...
# make sure the coverage directory exists and is cleaned up
       ...
       system("find . -name '*.da' | xargs --no-run-if-empty /bin/rm");
  ...
  system("(find . -name '*.results.xml' -exec /bin/cp {} $outdir \\; 2>&1) > /dev/null");
  system("(find . -name '*.test.xml' -exec /bin/cp {} $outdir \\; 2>&1) > /dev/null");
  #
  ...
  if ($coverage) {
    ...
    #
    # genhtml using coverage directory as output (overwrites existing, but keeps
