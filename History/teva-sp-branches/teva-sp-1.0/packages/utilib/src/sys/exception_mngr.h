...
#define STD_CATCH(cmd)\
  catch (const char* str) { cerr << "ERROR! Caught exception string: " << str << endl << std::flush; cmd ; }\
\
  catch (const std::bad_alloc& err) { cerr << "ERROR! Caught `bad_alloc' exception: " << err.what() << endl << std::flush; cmd ; }\
\
  catch (const std::bad_exception& err) { cerr << "ERROR! Caught `bad_exception' exception: " << err.what() << endl << std::flush; cmd ; }\
\
  catch (const std::bad_cast& err) { cerr << "ERROR! Caught `bad_cast' exception: " << err.what() << endl << std::flush; cmd ; }\
\
  catch (const std::bad_typeid& err) { cerr << "ERROR! Caught `bad_typeid' exception: " << err.what() << endl << std::flush; cmd ; }\
\
  catch (const std::ios_base::failure& err) { cerr << "ERROR! Caught `ios_base::failure' exception: " << err.what() << endl << std::flush; cmd ; }\
\
  catch (const std::length_error& err) { cerr << "ERROR! Caught `length_error' exception: " << err.what() << endl << std::flush; cmd ; }\
\
  catch (const std::domain_error& err) { cerr << "ERROR! Caught `domain_error' exception: " << err.what() << endl << std::flush; cmd ; }\
\
  catch (const std::out_of_range& err) { cerr << "ERROR! Caught `out_of_range' exception: " << err.what() << endl << std::flush; cmd ; }\
\
  catch (const std::invalid_argument& err) { cerr << "ERROR! Caught `invalid_argument' exception: " << err.what() << endl << std::flush; cmd ; }\
\
  catch (const std::logic_error& err) { cerr << "ERROR! Caught `logic_error' exception: " << err.what() << endl << std::flush; cmd ; }\
\
  catch (const std::range_error& err) { cerr << "ERROR! Caught `range_error' exception: " << err.what() << endl << std::flush; cmd ; }\
\
  catch (const std::overflow_error& err) { cerr << "ERROR! Caught `overflow_error' exception: " << err.what() << endl << std::flush; cmd ; }\
\
  catch (const std::underflow_error& err) { cerr << "ERROR! Caught `underflow_error' exception: " << err.what() << endl << std::flush; cmd ; }\
\
  catch (const std::runtime_error& err) { cerr << "ERROR! Caught `runtime_error' exception: " << err.what() << endl << std::flush; cmd ; }\
\
  catch (const std::exception& str) { cerr << "ERROR! Caught other exception object: " << str.what() << endl << std::flush; cmd ; }
