...
public:
  ...
  PackBuffer& write(PackBuffer& pb) const;

  
  /// Unpack an Ereal from an \a UnPackBuffer.
  UnPackBuffer& read(UnPackBuffer& upb);
...
inline Ereal<Type>::operator Type () const
{
  ...
  if (val == -1.0)      // -infinity
    ...
    EXCEPTION_MNGR(invalid_argument, "Ereal:: Type (): Attempt to cast NaN to a numeric value.")
  else if (val == 0.0)  // indeterminate
    EXCEPTION_MNGR(invalid_argument, "Ereal:: Type (): Attempt to cast indeterminate value to a numeric value.")
  else                  // unknown, non-finite value
    EXCEPTION_MNGR(runtime_error,"Ereal:: Type (): Invalid internal state detected: val=" << val << ", finite=false")
  
  return NULL; //this should never get reached, but gets rid of a lot of compiler warning messages.
}
    ...
    if (yFinite) {  // x is finite, y is finite
      ...
      if (conservativeError) {
        if (yval == 2.0)                      // y is NaN
          EXCEPTION_MNGR(range_error, "Ereal::plus - addtion resulted in NaN.")
        else if (yval == 0.0)                 // y is indeterminate
          EXCEPTION_MNGR(range_error, "Ereal::plus - addtion resulted in an indeterminate value.")
        else if (yval == -1.0 || yval == 1.0) // y is -infinity or infinity
          rval = yval;
        else                                  // y is not a known, non-finite value
          EXCEPTION_MNGR(runtime_error, "Ereal::plus - Invalid internal state detected: val=" << yval << ", finite=" << yFinite << ".")
      }
      else {
    ...
    if (yFinite) {  // x is non-finite, y is finite
      if (conservativeError) {
        if (xval == 2.0)                      // x is NaN
          ...
          EXCEPTION_MNGR(runtime_error, "Ereal::plus - Invalid internal state detected: val=" << xval << ", finite=" << xFinite << ".")
        else
          rval = xval;                        // x is infinity or -infinity
      }
      else {
    ...
    if (yFinite) {  // x is finite, y is finite
      ...
      if (conservativeError) {
        if (yval == 0.0)                      // y is indeterminate
          EXCEPTION_MNGR(range_error, "Ereal::minus - subtraction resulted in an indeterminate value.")
        else if (yval == 2.0)                 // y is NaN
          EXCEPTION_MNGR(range_error, "Ereal::minus - subtraction resulted in NaN.")
        else if (yval == -1.0 || yval == 1.0) // y is (+/-)infinity
          rval = -yval;
        else                                  // y is not a known, non-finite value
          EXCEPTION_MNGR(runtime_error, "Ereal::minus - Invalid internal state detected: val=" << yval << ", finite=" << yFinite << ".")
      }
      else {
        if (yval != -1.0 && yval != 1.0 && yval != 0.0 && yval != 2.0)  // y is not a known, non-finite value
          EXCEPTION_MNGR(runtime_error, "Ereal::minus - Invalid internal state detected: val=" << yval << ", finite=" << yFinite << ".")
    ...
    if (yFinite) {  // x is non-finite, y is finite
      if (conservativeError) {
        if (xval == 0.0)                      // x is indeterminate
          ...
          EXCEPTION_MNGR(runtime_error, "Ereal::minus - Invalid internal state detected: val=" << xval << ", finite=" << xFinite << ".")
      }
      else {
        if (xval != -1.0 && xval != 1.0 && xval != 0.0 && xval != 2.0)  // x is not a known, non-finite value
          EXCEPTION_MNGR(runtime_error, "Ereal::minus - Invalid internal state detected: val=" << xval << ", finite=" << xFinite << ".")
			...
			Type& rval, bool& rFinite)
{
  if (xFinite) {
    if (yFinite) {  // x is finite, y is finite
      ...
      if (conservativeError) {
        if (yval == 2.0)                        // y is NaN
          EXCEPTION_MNGR(range_error, "Ereal::mult - multiplication resulted in NaN.")
        else if (yval == 0.0)                   // y is indeterminate
          EXCEPTION_MNGR(range_error, "Ereal::mult - multiplication resulted in an indeterminate value.")
        else if (yval == -1.0 || yval == 1.0) { // y is (+/-)infinity
          ...
          EXCEPTION_MNGR(runtime_error, "Ereal::mult - Invalid internal state detected: val=" << yval << ", finite=" << yFinite << ".")
      }
      else {
        if (yval != -1.0 && yval != 1.0 && yval != 0.0 && yval != 2.0)  // y is not a known, non-finite value
          EXCEPTION_MNGR(runtime_error, "Ereal::mult - Invalid internal state detected: val=" << yval << ", finite=" << yFinite << ".")
    ...
    if (yFinite) {  // x is non-finite, y is finite
      if (conservativeError) {
        if (xval == 2.0)                        // x is NaN
          ...
          EXCEPTION_MNGR(runtime_error, "Ereal::mult - Invalid internal state detected: val=" << xval << ", finite=" << xFinite << ".")
      }
      else {
        if (xval != -1.0 && xval != 1.0 && xval != 0.0 && xval != 2.0)  // x is not a known, non-finite value
          EXCEPTION_MNGR(runtime_error, "Ereal::mult - Invalid internal state detected: val=" << xval << ", finite=" << xFinite << ".")
		...
		Type& rval, bool& rFinite)
{
  if (xFinite) {
    if (yFinite) {  // x finite, y finite
      if (yval == 0.0)
        EXCEPTION_MNGR(domain_error,"Ereal::div - division by zero")
      else {
        rFinite = true;
        rval = xval / yval;
      }
      ...
      if (yval == -1.0 || yval == 1.0) {
        ...
        if (conservativeError) {
          if (yval == 0.0)
            EXCEPTION_MNGR(range_error,"Ereal::div - division resulted in an indeterminate number.")
          else if (yval == 2.0)
            EXCEPTION_MNGR(range_error,"Ereal::div - division resulted in NaN.")
        }
        else {
          rFinite = false;
          rval = yval;
        ...
        EXCEPTION_MNGR(runtime_error,"Ereal::div - invalid internal state detected: val=" << yval << ", finite=" << yFinite << ".")
    }
  }
  else {
    if (yFinite) {  // x non-finite, y finite
      if (conservativeError) {
        ...
        if (xval == 2.0)  // x is NaN
          ...
          EXCEPTION_MNGR(runtime_error,"Ereal::div - invalid internal state detected: val=" << xval << ", finite=" << xFinite << ".")
        else if (yval > 0.0)  //+-infinity/+finite
          rval = xval;
        else if (yval < 0.0)  //+-infinity/-finite
          rval = -xval;
  ...
  if (is >> s) {
    ...
    if (iss >> t) { // try to read a 'Type'
      ...
      if (s == "-Infinity" || s == "-Inf" || s == "-infinity" || s == "-inf")             // -infinity
        ...
        EXCEPTION_MNGR(runtime_error, "Ereal::read - Unrecognized value: " << s )
    }
  }
  else {
    EXCEPTION_MNGR(runtime_error,"Ereal::read - unable to read in value.")
  }
 ...
 * 
 * \param pb The \a PackBuffer to pack the Ereal into.
 *
 ...
 *
 * \param upb The \a UnPackBuffer to read from.
 *
...
operator+ (const Ereal<Type>& x)
{
  if (x.finite) {
    ...
    if (x.val == -1.0 || x.val == 1.0)
      ...
      EXCEPTION_MNGR(invalid_argument, "Ereal::operator+ - Attempted operation on an indeterminate value.")
    else if (x.val == 2.0)
      EXCEPTION_MNGR(invalid_argument, "Ereal::operator+ - Attempted operation on NaN.")
    else
      EXCEPTION_MNGR(runtime_error, "Ereal::operator+ - Invalid internal state detected: val=" << x.val << ", finite=false")
  }
...
Ereal<Type> operator- (const Ereal<Type>& num)
{
  if (num.Finite || num.val == -1.0 || num.val == 1.0)  // num is finite, -infinity, or +infinity
    ...
    EXCEPTION_MNGR(runtime_error, "Ereal::operator+ - Invalid internal state detected: val=" << num.val << ", finite=false.")
...
bool operator== (const Ereal<Type>& x, const Ereal<Type>& y)
{
  if (x.Finite) {
    if (y.Finite) {
      ...
      if (y.val == 0.0)
        EXCEPTION_MNGR(invalid_argument, "Ereal::operator== - indeterminate value used in equality comparison")
      else if (y.val == 2.0)
        EXCEPTION_MNGR(invalid_argument, "Ereal::operator== - NaN used in equality comparison.")
      else if (y.val != -1.0 && y.val != 1.0)
        EXCEPTION_MNGR(runtime_error, "Ereal::operator== - Invalid internal state detected: val =" << y.val << ", finite="<< y.Finite << ".")
      else
        return false;   
    }
  } 
    ...
    if (y.Finite) {
      if (x.val == -1.0 || x.val == 1.0) {
        ...
        if (x.val == 0.0)
          ...
          EXCEPTION_MNGR(runtime_error, "Ereal::operator== - Invalid internal state detected: val =" << x.val << ", finite="<< x.Finite << ".")
      }
    }
    else {
      if ((x.val == -1.0 || x.val == 1.0 ) && (y.val == -1.0 || y.val == 1.0))
        ...
        EXCEPTION_MNGR(runtime_error, "Ereal::operator== - Invalid internal state detected: xval =" << x.val << ", xFinite="<< x.Finite << ", yval=" << y.val << ", yFinite=" << y.Finite << ".")
    }
  }
  ...
  if (x.Finite) {
    if (y.Finite) {           // finite < finite
      ...
      if ( y.val == -1.0)     // finite < -infinity
        ...
        EXCEPTION_MNGR(invalid_argument, "Ereal - Attempted relational operation using an indeterminate value.")
      else if (y.val == 2.0)  // finite < NaN
        EXCEPTION_MNGR(invalid_argument, "Ereal - Attempted relational operation using NaN.")
      else                    // finite < invalid
        EXCEPTION_MNGR(runtime_error, "Ereal - invalid internal state detected: val=" << y.val << ", finite=false")
    }
  }
  else {
    if (y.Finite) {
      if (x.val == -1.0)      // -infinity < finite
        ...
        EXCEPTION_MNGR(runtime_error, "Ereal - invalid internal state detected: val=" << x.val << ", finite=false")
    }
    else {
      if ( (x.val == -1.0 || x.val == 1.0) && (y.val == -1.0 || y.val == 1.0) )
      ...
      if (x.val == 0.0 || y.val == 0.0)       // x and/or y is indeterminate
        ...
        EXCEPTION_MNGR(runtime_error, "Ereal - invalid internal state detected: xval=" << x.val << ", xFinite=" << x.Finite << ", yval=" << y.val << ", yFinite=" << y.Finite)
    }
  }
...
long int lround(Ereal<Type>& num)
{
  if ( finite(num) ) {
    ...
    if (num == Ereal<Type>::negative_infinity)
      ...
      EXCEPTION_MNGR(invalid_argument, "Eread::lround: Attempt to round an indeterminate value to an integer")
    else if (num == Ereal<Type>::NaN)
      EXCEPTION_MNGR(invalid_argument, "Eread::lround: Attempt to round NaN to an integer.")
    else
      EXCEPTION_MNGR(runtime_error, "Eread::lround: Invalid internal state detected:" << num)
  }
