...
/// Out-stream operator
template <class T, class InfoT>
inline ostream& operator<<(ostream& output, const utilib::SimpleHashTableItem<T,InfoT>& item)
{ item.write(output); return(output); }


/// In-stream operator
template <class T, class InfoT>
inline istream& operator>>(istream& input, utilib::SimpleHashTableItem<T,InfoT>& item)
{ item.read(input); return(input); }
#endif
