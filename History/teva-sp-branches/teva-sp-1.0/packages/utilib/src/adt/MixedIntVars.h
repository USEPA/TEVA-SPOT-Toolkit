...
protected:
  ...
  /// of variables.
  explicit MixedIntVarsRep(int nBinVars=0, int nIntVars=0, int nDblVars=0) 
	: ref(1), 
	own(AssumeOwnership),
...
public:
   ...
   * of variables.
   */
  explicit MixedIntVars(const int numBits=0, const int numGenInts=0, 
   ...
   * variables, which precede real variables.
   */
  double get(const int ndx) const;
...
inline utilib::UnPackBuffer& operator>> (utilib::UnPackBuffer& buff, 
					utilib::MixedIntVars& vars)
{ vars.read(buff); return buff; }

inline ostream& operator<< (ostream& buff, const utilib::MixedIntVars& vars)
{ vars.write(buff); return buff; }

inline istream& operator>> (istream& buff, utilib::MixedIntVars& vars)
{ vars.read(buff); return buff; }   


inline vector<double>& operator<< (vector<double>& buff, 
					const utilib::MixedIntVars& vars)
{
...
inline vector<int>& operator<< (vector<int>& buff, 
					const utilib::MixedIntVars& vars)
{
