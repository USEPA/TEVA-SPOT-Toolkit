...
class PMSearch 
{
 ...
 public:
  ...
  virtual double search(PMSolution *s, bool best_improvement, PMElite *elite, bool use_list, double *nz = 0, double *profile=0);
  ...
  static void fastInterchange (PMSolution *s, int *itcount = 0);
  static void localSearch (PMSolution *s, bool best_improvement, bool uselist, int *itcount=0, bool stats=false, bool output_density=false, double *profile=0);
  static void update (PMSolution *s, int in, int out);
  static void runLocalSearch (PMSolution *s, char *lsmethod, bool output_density = false, double *profile = 0);
};
