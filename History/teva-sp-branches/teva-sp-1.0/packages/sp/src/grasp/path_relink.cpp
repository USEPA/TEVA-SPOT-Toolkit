...
double PMRelink::combinePR (PMSolution *t, PMSolution *s1, PMSolution *s2) {
  ...
  t->copyFrom(*s1);
  sm->search (t, true, single_elite, false); //perform path-relinking
...
void PMRelink::pathRelinking (PMSolution *best_s, PMElite *elite, const char *method, bool stats) 
{
  ...
  while (advance) {
	...
	if (r->computeDifference(source)>1 && r->computeDifference(target)>1) {
	  PMSearch::localSearch (r, true, false);  //try to improve solution with local search
