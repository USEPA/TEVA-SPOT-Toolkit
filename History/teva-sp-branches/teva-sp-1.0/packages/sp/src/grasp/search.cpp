...
void PMSearch::localSearch(PMSolution *s, bool best_improvement, bool uselist, int *itcount, bool stats, bool output_density, double *profile) 
{
  ...
  sm->search(s,best_improvement,0,uselist,output_density ? &density : 0,profile);
  if(verbose)
    {
...
void PMSearch::update(PMSolution *s, int in, int out) 
{ 
...
void PMSearch::runLocalSearch(PMSolution *s,char *localSearchMethod,bool output_density,double *profile) 
{
  for(int i=0;localSearchMethod[i]!='\0';i++) 
	...
	case 'm': // matrix
	  ...
	  localSearch(s,true,false,0,false,output_density,profile); 
	  break; 
...
void PMSearch::move(PMSolution *s,int in,int &out,double &w) 
{
  ...
  for(int i=1;i<=p;i++) 
    {
      if(v[s->getFacility(i)]<v[best])
	{
	  best=s->getFacility(i);
	}
    }
