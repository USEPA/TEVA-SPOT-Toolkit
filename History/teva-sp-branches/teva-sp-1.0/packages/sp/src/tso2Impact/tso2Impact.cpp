...
/// The main function for tso2Impact.
...
" << std::endl;
      ...
      std::string tsoFileOrDirectoryName(argv[2]);

      std::cout << "TSO file/directory=" << tsoFileOrDirectoryName << std::endl;
  
      int detectionDelay(0); // default is 0
      double minQuality(0.0); // default is off
      ...
      for(int i=0;i<net->nnodes;i++)
	{
      ...
      std::string nodeMapFileName(outputFilePrefix+"."+NODE_MAP_FILE_SUFFIX);
      std::string scenarioMapFileName(outputFilePrefix+"."+SCENARIO_MAP_FILE_SUFFIX);

      std::ofstream nodeMapFile(nodeMapFileName.c_str()); 
      std::ofstream scenarioMapFile(scenarioMapFileName.c_str()); 

      std::vector<std::string> nodeIndexToIDMap(numNodes); // maps 0-based TEVA node indicies to node textual IDs
      ...
      for(int i=0;i<net->nlinks;i++)
	{
      ...
      int numScenarios=0; // a 1-based index
      ...
      if(ecSelected==true)
	{
      ...
      if(pkSelected==true)
	{
	  PopulationKilledObjective *pkObj=new PopulationKilledObjective;
	  pkObj->setTAIFileName(taiInputFilename.c_str());
	  pkObj->setTSOFileOrDirectory(tsoFileOrDirectoryName.c_str());
	  if(tsoPattern!="")
	    {
	      pkObj->setTSOPattern(tsoPattern.c_str());
	    }
	  theObjectives.push_back(pkObj);
	}
      if(peSelected==true)
	{
	  PopulationExposedObjective *peObj=new PopulationExposedObjective;
	  peObj->setTAIFileName(taiInputFilename.c_str());
	  peObj->setTSOFileOrDirectory(tsoFileOrDirectoryName.c_str());
	  if(tsoPattern!="")
	    {
	      peObj->setTSOPattern(tsoPattern.c_str());
	    }
      ...
      for(size_t i=0;i<theObjectives.size();i++)
	{
      ...
      ///////////////////////////////////////
      // process impacts for each scenario //
      ///////////////////////////////////////
