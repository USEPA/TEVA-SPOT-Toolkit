			   ...
			   double *profile) 
{
  ...
  sm->search(s,facilityLocationConstraints,best_improvement,0,uselist,output_density ? &density : 0,profile);
  if(verbose)
    {
			      ...
			      double *profile)
{
  for(int i=0;localSearchMethod[i]!='\0';i++) 
	...
	case 'f': // fast interchange 
	  ...
	  fastInterchange(s,facilityLocationConstraints); 
	  break;                                 

	case 'm': // matrix

	  localSearch(s,true,false,facilityLocationConstraints,0,false,output_density,profile); 
	  break; 
