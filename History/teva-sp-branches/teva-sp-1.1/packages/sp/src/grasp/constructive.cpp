...
void PMConstructive::run(PMSolution *s, int k) 
{
  ...
  switch(m) 
    ...
    case RGREEDY : greedy(s,k,false,v,0); break;
    case PGREEDY : greedy(s,k,true,v,0); break;
    case PWORST  : pworst(s,k,0,v,false); break;
...
void PMConstructive::rpg(PMSolution *s,int k,double fractionRandom,bool stats) 
{
  ...
  int numRandom((int)(ceil(fractionRandom*double(k))));

  // construct the solution
  s->reset();             // start with an empty solution
  addRandom(s,numRandom); // add numRandom elements at random
  greedy(s,k,false,0,0);  // complete greedily
