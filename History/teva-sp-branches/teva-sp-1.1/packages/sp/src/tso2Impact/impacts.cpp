  ...
  if (scs.size()) {
    for (size_t j=0;j<sensors.size();j++) {
      if(theseImpacts.find(sensors[j])!=theseImpacts.end())
	    {
	      expectedImpact += theseImpacts[sensors[j]] *
		remainingProbability * sc_probabilities[scs[sensors[j]]];
	      remainingProbability -= remainingProbability *
		sc_probabilities[scs[sensors[j]]];
	    }
    }
    expectedImpact += minImpact * remainingProbability;
    stats.impact[i] = values[i] = expectedImpact;
  } else {
    for (size_t j=0;j<sensors.size();j++) {
      if(theseImpacts.find(sensors[j])!=theseImpacts.end())
	    {
