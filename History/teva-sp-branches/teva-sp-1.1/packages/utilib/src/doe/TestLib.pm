...
sub process_testoptions {
  ...
  #
  $expoptions = "";
  #@sortedauxlist = sort { $a cmp $b } @auxlist;
  foreach $i (@auxlist) {
       $expoptions .= $i . " ";
  }
  #
...
sub generate_description {
  ...
  while (@_) {
    ...
    local $tmp = shift(@_);
    scrub(\$tmp);
    $description .= "    <Factor id=\"" . $id . "\">" . $tmp . "</Factor>\n";
    }
...
sub exec_configure {
  ...
  if ($logfile eq "") {
     ...
     if (@_) {
        $status = 0xffff & system("(./configure $siteconfig @_) > ${outdir}/config.out 2>&1");
     } else {
        $status = 0xffff & system("(./configure $siteconfig) > ${outdir}/config.out 2>&1");
     }
...
sub exec_build {
  ...
  if ($logfile eq "") {
     ...
     print LOGFILE "<hr><pre>\n";
     close LOGFILE;
     $status = 0xffff & system("($clean make ; make executables) >> ${outdir}/build.out 2>&1");
