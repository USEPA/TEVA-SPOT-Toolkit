...
public:
  ...
  void debug_print(ostream& str,int max_elements=0);
...
protected:
  ...
  /// Default is nothing.
  virtual void dump_data(ostream& /*str*/, unsigned int /*max_elements*/=0)
	{}
...
template <class A, class P>
void ArrayBase<A,P>::debug_print(ostream& str,int max_elements)
{
  str << "this=" << this;
  ...
  if (prev_share == special_not_owned_pointer())
    str << "(not owned flag)";
  str << " next_share=" << next_share << endl;
