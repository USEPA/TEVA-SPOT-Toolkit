...
static char *version = "GLPSOL -- GLPK LP/MIP Solver, Version 4.8";
/* version string */
...
{     LPX *lp;
      ...
      if (memlim >= 0 && memlim < INT_MAX / (1024 * 1024))
         lib_env_ptr()->mem_limit = (1024 * 1024) * memlim;
...
err:        {  print("Model processing error");
      ...
      if (method == 0 && basis && !presol && in_bas == NULL)
         lpx_adv_basis(lp);
