...
public:
  ...
  // WEH - I'm not sure why the SGI compiler
		...
		//	 declaration.
  typedef 
  #if !defined(SGI_CC) && !defined(OSF_CC)
  const
  #endif
...
protected:
  ...
  bool is_negative(T data) const {return (data < 0);}
};


/// Specialization for unsigned ints
template <>
class BasicArrayBounds<unsigned int>
{
protected:

  /// Returns true if the array index is negative
  bool is_negative(unsigned int) const {return false;}
};

/// Specialization for unsigned long ints
template <>
class BasicArrayBounds<unsigned long int>
{
protected:

  /// Returns true if the array index is negative
  bool is_negative(unsigned long int) const {return false;}
};
...
if (is_negative(idx) || ((size_type)idx >= Len))
