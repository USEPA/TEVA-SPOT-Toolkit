...
void BCMip::reset(bool resetVB)
{
  ...
  // CGL Probing cut finder will throw an exception (failed assertion)
  // if this condition isn't met.  In particular, it fails on 64-bit machines.
  // We hope that CGL will fix this, but for now, this will work around.

  if (sizeof(size_t) != sizeof(unsigned int))
    {
      ucout << "WARNING: CGL probing cuts disabled.  They will not work on this architecture.\n";
