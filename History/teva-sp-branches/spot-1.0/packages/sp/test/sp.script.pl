...
##
if ($ARGV[0] eq "-tso") {
   $cmdline = "../../../bin/threat_sim " . $factor{"tsg_file"} . " " . $factor{"tso_file"} . " " . $factor{"epanet_file"} . " > $ARGV[3]";
...
##
   ...
   $cmdline = "../../../bin/tso2Impact " . $factor{"impact_prefix"} . " " . $factor{"objectives"} . "> $ARGV[3]";
   print "Running command: $cmdline\n";
   `$cmdline > $ARGV[3]`;
   open OUTPUT, ">$ARGV[2]";
   print OUTPUT "_exit_status numeric/integer 0\n";
...
## SP-SIMPLE
##
elsif (($ARGV[0] eq "-sp-simple") || ($ARGV[0] eq "-sp") || ($ARGV[0] eq "-sp-timing") || ($ARGV[0] eq "-sp-study")) {

   if ($ARGV[0] eq "-sp") {
     $cmdline = "../../../bin/sp --path=../../../bin --path=../mod --print-log " . $factor{"impact_prefix"} . " " . $factor{"goals"} . " " . $factor{"solver"};

   } elsif ($ARGV[0] eq "-sp-study") {
     $cmdline = "../../../bin/sp --path=../../../bin --path=../mod --print-log " . $factor{"impact_prefix"} . " " . $factor{"goals"} . " " . $factor{"solver"} . " " . $factor{"sensors"};

   } elsif ($ARGV[0] eq "-sp-simple") {
     $objective = $goal . "_" . $measure;
     $cmdline = "../../../bin/sp --path=../../../bin --path=../mod --print-log --objective=" . $factor{"goals"} . "_" . $factor{"measure"} . " " . $factor{"impact_prefix"} . " " . $factor{"sensors"} . " " . $factor{"costs"} . " " . $factor{"format"} . " " . $factor{"summary"} . " " . $factor{"solver"} . " --seed=" . $option{"seed"} . " " . $factor{"threshold"} . " " . $factor{"gamma"} . " " , $factor{"side_constraint"} . " " . $factor{"sensor-locations"};
     #$cmdline = "../../../bin/sp --path=../../../bin --path=../mod --print-log --objective=" . $factor{"goals"} . "_" . $factor{"measure"} . " " . $factor{"impact_prefix"} . " " . $factor{"sensors"} . " " . $factor{"solver"} . " --seed=" . $option{"seed"};
   } else {
     $objective = $goal . "_" . $measure;
     $cmdline = "../../../bin/sp --path=../../../bin --path=../mod --print-log --objective=" . $factor{"goals"} . "_" . $factor{"measure"} . " " . $factor{"impact_prefix"} . " " . $factor{"solver"} . " --seed=" . $option{"seed"};
   }
   print "Running command: $cmdline\n";
   `$cmdline > $ARGV[3]`;
   open OUTPUT, ">$ARGV[2]";
