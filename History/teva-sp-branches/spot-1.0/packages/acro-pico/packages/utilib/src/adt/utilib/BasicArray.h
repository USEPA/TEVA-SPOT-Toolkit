...
public:
  ...
  // WEH - I'm not sure why the SGI compiler
		...
		//	 declaration.
  typedef 
  #if !defined(SGI_CC) && !defined(OSF_CC) && !defined(__PGI) && !defined(_MSC_VER)
  const
  #endif
...
protected:
  ...
  /// Returns true if the array index is negative
  bool is_negative(const unsigned int) const {return false;}
};
...
protected:
  ...
  /// Returns true if the array index is negative
  bool is_negative(const unsigned long int) const {return false;}
};
