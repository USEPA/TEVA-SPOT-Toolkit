...
public:  
  ...
  void packSolution(PackBuffer& outBuffer)    { incumbent.pack(outBuffer);  };
  void unpackSolution(UnPackBuffer& inBuffer) { incumbent.unpack(inBuffer); };

  int  solutionBufferSize() { return incumbent.maxPackSize(); };
