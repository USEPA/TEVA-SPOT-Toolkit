...
using utilib::GenericSplayTree;
using utilib::GenericSplayTreeItem;
using utilib::Ereal;
...
protected:
  ...
  GenericSplayTree<PointList<PointT> > tree;
...
if (!tree || (std::fabs(tree.top()->key().val -val) > value_tolerance)) {
   PointList<PointT>* tlist = new PointList<PointT>();
   tlist->val = val;
   tree.add(*tlist);
...
//
if ((EnumLimit > 0) && ((val - worst_val)*sense < 0.0) && (num >= (EnumLimit+worst_num))) {
   tmp.val = worst_val;
   bool status;
   tree.splay(tmp);
...
const GenericSplayTreeItem<PointList<PointT> >* node = tree.find_rank(0);
