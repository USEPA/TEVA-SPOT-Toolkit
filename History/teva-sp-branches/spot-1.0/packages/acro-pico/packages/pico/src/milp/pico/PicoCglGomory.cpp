...
// PicoCglGomory.hpp
//
// A frozen version of the COIN-OR CGL Gomory cut code, modified to return
// globally valid cuts.
//
// Cindy Phillips
//
...
void picoCglGomory::generateCuts(const OsiSolverInterface & si, OsiCuts & cs,
			     const CglTreeInfo info) const
{
  ...
  if (intVar == NULL)
    EXCEPTION_MNGR(std::runtime_error, "Out of memory in PicoCglGomory\n");
#endif
  int i;
  ...
  for (i=0;i<numberColumns;i++) {
	...
	intVar[i]=1; //0-1
      ...
      else if (colUpper[i]>colLower[i]+0.5) {
	if (colLower[i]>=0.0) {
	  intVar[i] = 2; // other
	} else {
  ...
  generateCuts(debugger, cs, *si.getMatrixByCol(),
	   ...
	   intVar,warm, si.getInfinity(), info);
...
int
picoCglGomory::generateCuts( const OsiRowCutDebugger * debugger, 
                         OsiCuts & cs,
                         ...
                         const CglTreeInfo info) const
{
  ...
  double bounds[2]={-solverInfinity,0.0};
  int iColumn,iRow;
  ...
  for (iColumn=0;iColumn<numberColumns;iColumn++) {
    // EL: change made for global validity
      if(columnIsBasic[iColumn]<0) 
          if (intVar[iColumn] == 1) {
        if(1-colsol[iColumn] <= tolerance) {
             swap[iColumn]=true;
    } else {
      swap[iColumn]=false;
    } } else if (colUpper[iColumn]-colsol[iColumn]<=tolerance) {
      swap[iColumn]=true;
    } else {
      swap[iColumn]=false;
    }
  ...
  */
  ...
  for (iRow=0;iRow<numberRows;iRow++) {
	...
	// CAP: This is new in 2/06.  Seems to negate the test, but I guess you
	// can stop a debugger on it.
	//abort();
	continue;
#else
	continue;
...
// Limit stuff
void picoCglGomory::setLimit(int limit)
{
  if (limit>0)
    limit_=limit;
}
int picoCglGomory::getLimit() const
{
  return limit_;
}
// Limit stuff at root
void picoCglGomory::setLimitAtRoot(int limit)
{
  if (limit>0)
    limitAtRoot_=limit;
}
int picoCglGomory::getLimitAtRoot() const
{
  return limitAtRoot_;
}

// Away stuff
void picoCglGomory::setAway(double value)
{
  if (value>0.0&&value<=0.5)
    away_=value;
}
double picoCglGomory::getAway() const
{
  return away_;
}
...
// Copy constructor 
//-------------------------------------------------------------------
picoCglGomory::picoCglGomory (const picoCglGomory & source) :
...
//-------------------------------------------------------------------
CglCutGenerator *
picoCglGomory::clone() const
{
  return new picoCglGomory(*this);
}
...
//-------------------------------------------------------------------
picoCglGomory &
picoCglGomory::operator=(const picoCglGomory& rhs)
{
  if (this != &rhs) {
...
bool 
picoCglGomory::needsOptimalBasis() const
{
  return true;
}
...
std::string
picoCglGomory::generateCpp( FILE * fp) 
{
  picoCglGomory other;
