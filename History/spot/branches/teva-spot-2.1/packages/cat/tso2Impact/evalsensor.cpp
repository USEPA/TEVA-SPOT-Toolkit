...
int main(int argc, char **argv)
{
   try
   {
      ...
      options.usage = "evalsensor [options...] <sensor-file> <impact-file1> [<impact-file2>...]";
      ...
      options.add("debug", debug, "A boolean flag that adds output information about each event.");
 ...
 xml   Generates an XML-formated output that\n\
      ...
      options.add("event-probabilities", probabilityFilename, "A file with the probability of the different contamination scenarios.");
      ...
      options.add_argument("sensor-file", "A sensor placement file, which contains one or more sensor placements that will be evaluated.");
      options.add_argument("impact-file", "A file that contains the impact data concerning a contamination event.  If one or more impact files are specified, then evaluations are performed for each impact separately.");
...
utilib::OptionParser::args_t args = options.parse_args(argc,argv);
   ...
   if (options.version_option())
   {
      ...
      for (; fcurr != fend; fcurr++, scurr++, dicurr++, ascurr++)
      {
         ...
         // an associated file of event probabilities.  These are typically
         // redundant across different impacts (mc, vc, etc.).
         std::map<int, double> probabilities;
         std::string probFilename = *fcurr + ".prob";
         std::ifstream probabilityInputFile(probFilename.c_str());
         if (probabilityInputFile)
         {
            read_probabilities(probabilityInputFile, probabilities);
         }
         //
         ...
         while (mcurr != mend)
         {
            ...
            if (sensors.size() == 1) {
               compute_greedy_order(mcurr->second, *dicurr, *ascurr,
                                 scs, sc_probabilities, tmp, probabilities, work,
      ...
      if (format == "cout")
      {
         ...
         while (mcurr != mend)
         {
            ...
            if (sensors.size() == 1) {
               write_greedy_order(greedySensorOrders, files);
            }
            flag = false;
            mcurr++;
         }
