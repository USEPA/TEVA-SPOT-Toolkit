...
MSEPANET.C --	Hydraulic & MultiSpecies Water Quality Simulator 
...
See TOOLKIT.H and MSTOOLKIT.H for function prototypes of exported DLL functions
See FUNCS.H and MSFUNCS.H for prototypes of all other functions
See TYPES.H and MSTYPES.H for declaration of global constants and data structures
See VARS.H and MSVARS.H for declaration of global variables
See TEXT.H and MSTEXT.H for declaration of all string constants
See ENUMSTXT.H and MSENUMSTXT.H for assignment of string constants to enumerated types
...
5. Declared constants and enumerated values defined in TYPES.H and MSTYPES.H 
   are all uppercase.
6. String constants defined in TEXT.H and MSTEXT.H begin with a lower case 
   character followed by an underscore and then all uppercase characters 
   (e.g.t_HEADLOSS)
...
#define MULTISPECIES       /* Comment this line to turn off Multispecies functionality*/
...
*/
{
   *count = 0;
   ...
   switch (code)
   {
	  ...
	  case EN_SPECIESCOUNT: *count = Nspecies;  break;
	  case EN_BULKSPECIESCOUNT: *count = Nbulkspecies;  break;
      default: return(251);
   }
   return(0);
}
...
/*** Updated 11/19/01 ***/
	  ...
	  /******** Added 04/05/2005 Tom Taxon****************/
      case EN_LINKQUAL:
         *value = avgqual(index) * Ucf[LINKQUAL];
         break;
      /***************************************************/   
      default: return(251);
   }
...
/*
----------------------------------------------------------------
   Functions for retrieving multi speices data
----------------------------------------------------------------
*/
int DLLEXPORT ENMSgetspeciesindex(char *id, int *index)
/*----------------------------------------------------------------
**  Input:   id = species ID
**  Output:  *index = index of spcies 
**  Returns: error code                              
**  Purpose: retrieves index of a species with specific ID 
**----------------------------------------------------------------
*/
{
   *index = 0;
   if (!Openflag) return(102);
   *index = findspecieID(Specie, id, Nspecies);
   if (*index == 0) return(261);
   else return(0);
}

int DLLEXPORT ENMSgetspeciesid(int index, char *id)
/*----------------------------------------------------------------
**  Input:   index = index of species                  
**  Output:  id = specie ID
**  Returns: error code                              
**  Purpose: retrieves ID of a species with specific index
**
**  NOTE: 'id' must be able to hold MAXID characters
**----------------------------------------------------------------
*/
{
   ...
   if (Qualflag == MULTIS)
   {
	   if (index < 1 || index > Nspecies) return(261);
...
int  DLLEXPORT ENMSgetspeciestype(int index, int *code)
/*----------------------------------------------------------------
**  Input:   index = species index                    
**  Output:  *code = species type code number (see TOOLKIT.H)
**  Returns: error code                              
**  Purpose: retrieves species type of specific species 
**----------------------------------------------------------------
*/
{
   *code = -1;
   if (!Openflag) return(102);
   if (index < 1 || index > Nspecies) return(261);
   if (index <= Nbulkspecies) *code = EN_BULK;
   else if ( index <= Nbulkspecies+Npipesurfacespecies) *code = EN_PIPESURFACE;
   else *code = EN_TANKSURFACE;
   return(0);
}

int DLLEXPORT ENMSgetparameterindex(char *id, int *index)
/*---------------------------------------------------------------------
**  Input:   id = parameter ID
**  Output:  *index = index of parameter 
**  Returns: error code                              
**  Purpose: retrieves index of a reaction parameter with specific ID 
**---------------------------------------------------------------------
*/
{
   *index = 0;
   if (!Openflag) return(102);
   *index = findparameterID(id);
   if (*index == 0) return(262);
   else return(0);
}

int DLLEXPORT ENMSgetparameterid(int index, char * id)
/*----------------------------------------------------------------
**  Input:   index = index of parameter                  
**  Output:  id = parameter ID
**  Returns: error code                              
**  Purpose: retrieves ID of a multi species reaction parameter 
**
**  NOTE: 'id' must be able to hold MAXID characters
**----------------------------------------------------------------
*/
{
   strcpy(id,"");
   if (!Openflag) return(102);
   if (index < 1 || index > Nparameters) return(262);
   strcpy(id,Parameter[index].ID);
   return(0);
}

int DLLEXPORT ENMSgetnodequality(int nindex, int sindex, int code, float *value)
/*----------------------------------------------------------------
**  Input:   nindex = node index 
		     sindex = specie index                   
**  Output:  *value = node water quality
**  Returns: error code                              
**  Purpose: retrieves multi species quality at nodes
**----------------------------------------------------------------
*/
{
	Pminitial msI;
	...
	int findflag = 0;
	if (!Openflag) return(102);
	if (nindex <= 0 || nindex > Nnodes) return(203);
    if (sindex <= 0 || sindex > Nbulkspecies) return(260);
	switch (code)
	{
		case EN_INITQUAL:
			*value = 0.0;
			for (msI = NodeInitial[nindex]; msI != NULL; msI = msI->next)
			{
				if (msI->index == sindex) 
				{
					if (Specie[sindex].MSQtype == VOLUMEUNIT)
						*value = msI->C0*Ucf[QUALITY];
					else
						*value = msI->C0*UcfAreaq;
					break;
				}
			}
			break;
		...
		case EN_SOURCEMASS:
		case EN_SOURCEPAT:
			for (msS = NodeSource[nindex]; msS != NULL; msS = msS->next)
			{
				if (msS->index == sindex)
				{
					findflag = 1;
			...
			if (findflag == 0) return(240);
			if (code == EN_SOURCEQUAL)      *value = msS->C0;
			else if (code == EN_SOURCEMASS) *value = msS->Smass*60.0;
			else if (code == EN_SOURCEPAT)  *value = msS->Pat;
			else                            *value = msS->Type;
			return(0);
		case EN_QUALITY:
			*value = MSC[nindex][sindex]*Ucf[QUALITY];
			break;
		default:
			return (251);
	}
...
int DLLEXPORT ENMSsetnodequality(int nindex, int sindex, int code, float value)
/*----------------------------------------------------------------
**  Input:   nindex = node index 
		     sindex = specie index                   
			 value = node water quality
	Output:  none
**  Returns: error code                              
**  Purpose: set multi species quality at nodes
**----------------------------------------------------------------
*/
{
	Pminitial msI;
	...
	if (value < 0.0) return(202);
	if (nindex <= 0 || nindex > Nnodes) return(203);
    if (sindex <= 0) return(260);
	switch (code)
	{
		case EN_INITQUAL:
            if (sindex > Nspecies) return(260);
			for (msI = NodeInitial[nindex]; msI != NULL; msI = msI->next)
			{
				if (msI->index == sindex) 
				{
			...
			if (findflag == 0)
			{
				msI = (Pminitial)malloc(sizeof(struct Sminitial));
				if (msI == NULL) return (101);
				msI->next = NodeInitial[nindex];
				msI->index = sindex;
				NodeInitial[nindex] = msI;
			}
			if (Specie[sindex].MSQtype == VOLUMEUNIT)
				msI->C0 = value/Ucf[QUALITY];
			else
				msI->C0 = value/UcfAreaq;
			break;
		case EN_SOURCEQUAL:
		case EN_SOURCETYPE:
		case EN_SOURCEPAT:
            if (sindex > Nbulkspecies) return(260);
			for (msS = NodeSource[nindex]; msS != NULL; msS = msS->next)
			{
				if (msS->index == sindex)
				{
			...
			if (findflag == 0) 
			{
				msS = (Pmsource) malloc(sizeof(struct Smsource));
				if (msS == NULL) return(101);
				msS->Type = CONCEN;
				msS->C0 = 0.0;
				msS->Pat = 0;
				msS->index = sindex;
				msS->next = NodeSource[nindex];
				NodeSource[nindex] = msS;
			}
			if (code == EN_SOURCEQUAL) msS->C0 = value;
			else if (code == EN_SOURCEPAT)
			{
				j = ROUND(value);
				if (j < 0 || j > Npats) return(205);
...
int DLLEXPORT ENMSgetlinkquality(int lindex, int sindex, float *value)
/*----------------------------------------------------------------
**  Input:   lindex = linl index 
		     sindex = specie index                   
**  Output:  *value = averaged link water quality
**  Returns: error code                              
**  Purpose: retrieves averaged multi species quality within links
**----------------------------------------------------------------
*/
{
	int index;
	...
	index = StoPS[sindex];
    if (index <= 0 || index > Npipespecies) return(261);
	if (Specie[sindex].MSQtype == VOLUMEUNIT)
		*value = msavgqual(lindex, sindex)*Ucf[QUALITY];
	else 
		*value = msavgqual(lindex, sindex)*UcfAreaq;
	return (0);
}

int DLLEXPORT ENMSgetparametervalue(int code, int cindex, int pindex, float *value)
/*---------------------------------------------------------------------------------
**  Input:   code = component type code (EN_TANK|EN_PIPE) 
		     cindex = component index                 
			 pindex = parameter index
**  Output:  *value = parameter value
**  Returns: error code                              
**  Purpose: retrieves parametes values for multi species reaction dynamics 
**---------------------------------------------------------------------------------
*/
{
  *value = 0.0;
   ...
   if (code == EN_TANK)
   {
	   if (cindex <= 0 || cindex > Ntanks) return(203);
	   if (pindex <= 0 || pindex > Nparameters) return(262);
	   *value = TankParameters[cindex][pindex];
   }
   else if (code == EN_PIPE)
   {
	   if (cindex <= 0 || cindex > Npipes) return(204);
	   if (pindex <= 0 || pindex > Nparameters) return(262);
	   *value = LinkParameters[cindex][pindex];
   }
   return 0;

}

int DLLEXPORT ENMSsetparametervalue(int code, int cindex, int pindex, float value)
/*---------------------------------------------------------------------------------
**  Input:   code = component type code (EN_TANK|EN_PIPE) 
		     cindex = component index                 
			 pindex = parameter index
...
**  Output:  one
**  Returns: error code                              
**  Purpose: set parametes values for multi species reaction dynamics 
**---------------------------------------------------------------------------------
*/
{
   if (!Openflag) return(102);
   if (code == EN_TANK)
   {
	   if (cindex <= 0 || cindex > Ntanks) return(203);
	   if (pindex <= 0 || pindex > Nparameters) return(262);
	   TankParameters[cindex][pindex] = value;
   }
   else if (code == EN_PIPE)
   {
	   if (cindex <= 0 || cindex > Npipes) return(204);
	   if (pindex <= 0 || pindex > Nparameters) return(262);
	   LinkParameters[cindex][pindex] = value;
   }
   return 0;
...
int DLLEXPORT ENMSgetmassreact(int specieindex, float *value)
/*-------------------------------------------------------------------
**  Input:   specieindex = specie index                   
**  Output:  *value = mass reacted for species with index specieindex
**  Returns: error code                              
**  Purpose: retrieves mass loss due to reactions that are defined as
			 ODEs
**--------------------------------------------------------------------
*/
{
	if (specieindex <= 0 || specieindex > Nspecies)	return (260);
	*value = MSMassReact[specieindex];
	return 0;
}
...
*/      
      ...
      case 252:  sprintf(Msg,ERR252,"ENcreatepattern",""); break;
                                        
                                       /* MSEPANET errors */
      case 260:  sprintf(Msg,ERR260); break;
      case 261:  sprintf(Msg,ERR261); break;
      case 262:  sprintf(Msg,ERR262); break;
