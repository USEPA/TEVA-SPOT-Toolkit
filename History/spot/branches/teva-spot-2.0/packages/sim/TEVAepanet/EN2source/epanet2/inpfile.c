...
/* Write [CONTROLS] section */
   ...
   /* Get text of control's link status/setting */
      if (Control[i].Setting == MISSING)
         sprintf(s, " LINK %s %s ", Link[j].ID, StatTxt[(size_t)Control[i].Status]);
      else
      {
      ...
      /* Print level control */
         ...
         case HILEVEL:
            ...
            fprintf(f, "\n%s IF NODE %s %s %.4f", s,
               Node[n].ID, ControlTxt[(size_t)Control[i].Type], kc);
            break;
...
/* Write [TIMES] section */
   ...
   fprintf(f, "\n STATISTIC           %s",TstatTxt[(size_t)Tstatflag]);

/* Write [OPTIONS] section */

   fprintf(f, "\n\n[OPTIONS]");
   fprintf(f, "\n UNITS               %s", FlowUnitsTxt[(size_t)Flowflag]);
   fprintf(f, "\n PRESSURE            %s", PressUnitsTxt[(size_t)Pressflag]);                          
   fprintf(f, "\n HEADLOSS            %s", FormTxt[(size_t)Formflag]);
...
/* Write [REPORT] section */
   ...
   fprintf(f, "\n\n[REPORT]");
   fprintf(f, "\n PAGESIZE            %d", PageSize);
   fprintf(f, "\n STATUS              %s", RptFlagTxt[(size_t)Statflag]);
   fprintf(f, "\n SUMMARY             %s", RptFlagTxt[(size_t)Summaryflag]);
   fprintf(f, "\n ENERGY              %s", RptFlagTxt[(size_t)Energyflag]);
   switch (Nodeflag)
   {
      case 0:
      fprintf(f, "\n NODES               NONE");
