...
/*
 * Class:     anl_teva_db_TSOReader
 * Method:    open
 * Signature: (Ljava/lang/String;Ljava/lang/String;CC)I
 */
JNIEXPORT jint JNICALL Java_anl_teva_db_TSOReader_open
 ...
 * Signature: (I)V
 */
JNIEXPORT void JNICALL Java_anl_teva_db_TSOReader_positionAfterPrologue
 ...
 */
JNIEXPORT void JNICALL Java_anl_teva_db_TSOReader_readPrologue
 ...
 * Signature: (IIIII)V
 */
JNIEXPORT void JNICALL Java_anl_teva_db_TSOReader_closeAndFreeMemory
 ...
 * Signature: (IIII[[F)V
 */
JNIEXPORT jboolean JNICALL Java_anl_teva_db_TSOReader_readSimulationResults
  (JNIEnv *env, jobject reader, jint ptso, jint pNetInfo, jint pNodeInfo, jint pSource, jobjectArray jNodeInfo)
{
	jboolean hasData = TSO_ReadSimulationResults((PTSO)ptso,(PNetInfo)pNetInfo,(PNodeInfo)pNodeInfo,(PSource)pSource);
	if(hasData) {
 ...
 * Signature: (IIII[[F)V
 */
JNIEXPORT jboolean JNICALL Java_anl_teva_db_TSOReader_getSimulationResults
  (JNIEnv *env, jobject reader, jint id, jint ptso, jint pNetInfo, jint pNodeInfo, jint pSource, jobjectArray jNodeInfo)
{
	PTSO tso = (PTSO)ptso;

	jboolean hasData = TSO_GetSimulationResults(id,tso,(PNetInfo)pNetInfo,(PNodeInfo)pNodeInfo,(PSource)pSource);
	if(hasData) {
		x_populateNodeInfoAndSources(env,reader,(PNetInfo)pNetInfo,(PNodeInfo)pNodeInfo,(PSource)pSource, jNodeInfo);
	}
	return hasData;
 ...
 * Signature: (IIII[[F)V
 */
JNIEXPORT jboolean JNICALL Java_anl_teva_db_TSOReader_getRawSimulationResults
  (JNIEnv *env, jobject reader, jint id, jint ptso, jint pNetInfo, jint pNodeInfo, jint pSource, jobjectArray jNodeInfo, jbyteArray jbuf)
{
	...
	hasData = TSO_GetRawSimulationResults(id,tso,(PNetInfo)pNetInfo,(PNodeInfo)pNodeInfo,(PSource)pSource,rawData,bLen);
 ...
 * Class:     anl_teva_db_TSOReader
 * Method:    getIndex
 * Signature: (IIII)[Lanl/teva/db/DataIndex
 */
JNIEXPORT jobjectArray JNICALL Java_anl_teva_db_TSOReader_getIndex(
...
static jboolean x_initTSOData(JNIEnv *env)
{
	...
	if((x_tsoData->readerClass = x_findClass(env,"anl/teva/db/TSOReader","anl.teva.db.TSOReader"))==NULL)
		return FALSE;
	if((x_tsoData->netInfoClass = x_findClass(env,"anl/teva/db/TSONetInfo","anl.teva.db.TSONetInfo"))==NULL)
		return FALSE;
	if((x_tsoData->nodeInfoClass = x_findClass(env,"anl/teva/db/TSONodeInfo","anl.teva.db.TSONodeInfo"))==NULL)
		return FALSE;
	if((x_tsoData->linkInfoClass = x_findClass(env,"anl/teva/db/TSOLinkInfo","anl.teva.db.TSOLinkInfo"))==NULL)
		return FALSE;
	if((x_tsoData->sourceClass = x_findClass(env,"anl/teva/db/TSOSource","anl.teva.db.TSOSource"))==NULL)
		return FALSE;
	if((x_tsoData->dataIndexClass = x_findClass(env,"anl/teva/db/TSODataIndex","anl.teva.db.TSODataIndex"))==NULL)
		return FALSE;
	if((x_tsoData->compressedTSOClass = x_findClass(env,"anl/teva/db/CompressedTSOData","anl.teva.db.CompressedTSOData"))==NULL)
		return FALSE;
	...
	args = "(IIIIIIIIII[Ljava/lang/String;FIIIF)V";
	...
	args = "(IFF[F[[F[ILjava/lang/String;)V";
	...
	args = "(IIJI[Lanl/teva/db/TSOSource;)V";
	...
	if((x_tsoData->readerSources = x_findField(env,x_tsoData->readerClass,"sources","[Lanl/teva/db/TSOSource;","anl.teva.db.TSOReader"))==NULL)
		return FALSE;

	if((x_tsoData->nodeInfoQ = x_findField(env,x_tsoData->nodeInfoClass,"q","[F","anl.teva.db.TSONodeInfo"))==NULL)
		return FALSE;
	if((x_tsoData->nodeInfoC = x_findField(env,x_tsoData->nodeInfoClass,"c","[[F","anl.teva.db.TSONodeInfo"))==NULL)
		return FALSE;
	if((x_tsoData->nodeInfoNZ = x_findField(env,x_tsoData->nodeInfoClass,"nz","[I","anl.teva.db.TSONodeInfo"))==NULL)
		return FALSE;

	if((x_tsoData->linkInfoVelocity = x_findField(env,x_tsoData->linkInfoClass,"velocity","[F","anl.teva.db.TSOLinkeInfo"))==NULL)
		return FALSE;
	if((x_tsoData->linkInfoFlow = x_findField(env,x_tsoData->linkInfoClass,"flow","[F","anl.teva.db.TSOLinkeInfo"))==NULL)
		return FALSE;
	if((x_tsoData->linkInfoVX = x_findField(env,x_tsoData->linkInfoClass,"vx","[F","anl.teva.db.TSOLinkeInfo"))==NULL)
		return FALSE;
	if((x_tsoData->linkInfoVY = x_findField(env,x_tsoData->linkInfoClass,"vy","[F","anl.teva.db.TSOLinkeInfo"))==NULL)
		return FALSE;
...
static jobject x_CreateNetInfoObject(JNIEnv *env, PNetInfo pNetInfo) 
{
	...
	rv = (*env)->NewObject(env,x_tsoData->netInfoClass,x_tsoData->netInfoConstructor,
		...
		pNetInfo->reportstart, pNetInfo->reportstep, pNetInfo->fltmax);
	return rv;
}
static jobjectArray x_CreateNodeInfoArray(JNIEnv *env,PNetInfo pNetInfo,PNodeInfo pNodeInfo)
{
...
static jobject x_CreateNodeInfoObject(JNIEnv *env,PNodeInfo pNodeInfo, int nspecies, int nsteps)
{
	...
	return (*env)->NewObject(env,x_tsoData->nodeInfoClass,x_tsoData->nodeInfoConstructor,
		pNodeInfo->ntype,pNodeInfo->x,pNodeInfo->y,q,c,nz,id);
}
