...
void timingBasedDose(IndividualIngestionDataPtr *idp, IngestionDataPtr ingData, int detectTime, PNode node, PNet net, PU u, FILE *fp, PDoseOverThresh dot, int nodeidx,PDoseBinData dBins, float *avgDose);
void demandBasedDose(IndividualIngestionDataPtr idp, IngestionDataPtr ingData, int detectTime, PNode node, PU u, FILE *fp, PDoseOverThresh dot, int dotSize,PDoseBinData dBins, float *avgDose);
...
void DLLEXPORT Dose(IndividualIngestionDataPtr *idp, IngestionDataPtr ingData, PNode node, PNet net, int detectTime, PU u, PDoseOverThresh dot, int nodeidx, PDoseBinData dBins, float *avgDose)
{
	if((int)node->pop==0) return;
	if(ingData->timingMode == IMDemand) {
		demandBasedDose(idp[0],ingData,detectTime,node,u,NULL,dot,nodeidx,dBins,avgDose);
	} else {
		timingBasedDose(idp,ingData,detectTime,node,net,u,NULL,dot,nodeidx,dBins,avgDose);
	}
}
void demandBasedDose(IndividualIngestionDataPtr idp,IngestionDataPtr ingData, int detectTime,PNode node,PU u,FILE *fp, PDoseOverThresh dot, int nodeidx,PDoseBinData dBins, float *avgDose)
{
    int i;
