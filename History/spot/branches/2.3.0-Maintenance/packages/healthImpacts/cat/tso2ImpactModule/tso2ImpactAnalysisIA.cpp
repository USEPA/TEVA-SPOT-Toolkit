				    ...
				    NamedDataRef *intResultsRef)
{
	...
	for (int i=0; i<numInjections; i++) {
		...
		x_mem->sources[i].SourceStart     = sourceStartTimes[i];
		x_mem->sources[i].SourceStop      = sourceStopTimes[i];
		x_mem->sources[i].SourceStrength  = sourceStrengths[i];
	}
	...
	if(peSelected ||dpeSelected || pkSelected || dpkSelected || pdSelected|| dpdSelected) {
		HIST hist;
		int nsteps=x_mem->net->info->nsteps;
		...
		for(int i=0;i<nnodes;i++) {
		  ...
		  if(x_mem->node[i].info->nz[0] && pop > 0) {
		    Dose(ing->ingestionData[i],ing,&x_mem->node[i], x_mem->net,nsteps,x_mem->u,x_mem->dot,i,x_mem->doseBins,NULL);
	...
	processScenario(theObjectives, 0, detectionDelay, 1, minQuality,
			...
			x_mem->sources, x_mem->net->info->nsources, intResultsRef);
