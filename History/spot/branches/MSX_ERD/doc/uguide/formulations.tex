...
an implicit or explicit. However, in either case we can describe the
...
\item {\bfseries VaR:} Value-\/at-\/Risk (VaR) is a percentile-\/based metric. Given a confidence level $\beta\in(0,1)$, the VaR is the value of the distribution at the $1-\beta$ percentile \cite{TopVlaZen02}. The value of VaR is less than the TCE value.

Mathematically, suppose we have a random variable $W$ that describes the distribution of possible impacts. Then we have \[ {\rm VaR}(W,\beta) = \min \{ w \mid \Pr[W \le w] \ge \beta \}. \] Note that the distribution $W$ changes with each sensor placement. Further, VaR can be computed using the $\alpha$, $d$ and $x$ values.
