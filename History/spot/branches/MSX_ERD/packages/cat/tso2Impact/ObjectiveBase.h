...
public:
   ...
   virtual void finalizeForScenario(int scenarioIndex, PSourceData sources)
   {
   ...
   virtual void outputScenarioImpactData(int scenarioIndex, PSourceData sources)
   {
      if (myScenarioIDFile)
      {
         //
...
#endif
         if (sources != NULL)
            (*myScenarioIDFile) << scenarioIndex << " " << sources->source[0].sourceNodeIndex << " " << sources->source[0].sourceStart << std::endl;
      }
