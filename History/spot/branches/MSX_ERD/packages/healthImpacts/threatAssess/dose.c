...
void timingBasedDose(IndividualIngestionDataPtr *idp, IngestionDataPtr ingData, int detectTime,  PNode node, PNet net, PU u, FILE *fp, PDoseOverThresh dot, int dotSize,PDoseBinData dBins,int speciesIdx);
void demandBasedDose(IndividualIngestionDataPtr idp, IngestionDataPtr ingData, int detectTime, PNode node, PNet net, PU u, FILE *fp, PDoseOverThresh dot, int dotSize,PDoseBinData dBins, int speciesIdx);
...
LIBEXPORT(void) Dose(IndividualIngestionDataPtr *idp, IngestionDataPtr ingData, PNode node, PNet net, int detectTime, PU u, PDoseOverThresh dot, int nodeidx, PDoseBinData dBins, int speciesIdx)
{
	if((int)node->pop==0) return;
	if(ingData->timingMode == IMDemand) {
		demandBasedDose(idp[0],ingData,detectTime,node,net,u,NULL,dot,nodeidx,dBins,speciesIdx);
	} else {
		timingBasedDose(idp,ingData,detectTime,node,net,u,NULL,dot,nodeidx,dBins,speciesIdx);
	}
}
void demandBasedDose(IndividualIngestionDataPtr idp,IngestionDataPtr ingData, int detectTime,PNode node,PNet net, PU u,FILE *fp, PDoseOverThresh dot, int nodeidx,PDoseBinData dBins, int speciesIdx)
{
    int i;
...
void timingBasedDose(IndividualIngestionDataPtr *idp, IngestionDataPtr ingData, int detectTime, PNode node, PNet net, PU u, FILE *fp, PDoseOverThresh dot, int nodeidx,PDoseBinData dBins,int speciesIdx)
{
	float portionSize;
	int i,di,j;
