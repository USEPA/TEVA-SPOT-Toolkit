...
//	PLinkInfo links;
    ...
    if(!erdName) { /* temporary tso file */
        if( ( erdName = tmpnam( NULL ) ) == NULL )
...
int run_quality_msx(PERD db, FILE *tsifile, const char *msx_species, PSourceData source, double **initQual)
{
	...
	while ( ENL_setSource(source, net, tsifile,1) ) /* Load the next scenario define in TSI file */
	{
        ...
        do {
			...
			for (is=0; is < source->nsources; is++) {
				if ( entime >= sources[is].sourceStart && entime < sources[is].sourceStop ) { /* Source is on */
					ENCHECK( MSXsetsource(sources[is].sourceNodeIndex,sources[is].speciesIndex,sources[is].sourceType,sources[is].sourceStrength,0) );
				} else { /* Source is off */
					ENCHECK( MSXsetsource(sources[is].sourceNodeIndex,sources[is].speciesIndex,MSX_NOSOURCE,0,0) );
				}
			}
        	prevEntime=entime;
