...
JNIEXPORT void JNICALL Java_anl_msv_dpf_erd_db_TSOReader_readPrologue
  (JNIEnv *env, jobject reader, jlong ptso, jint flags, jobjectArray objects, jintArray ptrs)
{
	...
	TSO_ReadPrologue((PTSO)ptso,&pNetInfo,&pNodeInfo,&pLinkInfo,flags);
...
JNIEXPORT jboolean JNICALL Java_anl_msv_dpf_erd_db_TSOReader_getRawSimulationResults
  (JNIEnv *env, jobject reader, jint id, jlong ptso, jlong pNetInfo, jlong pNodeInfo, jlong pSource, jobjectArray jNodeInfo, jbyteArray jbuf)
{
	PTSO tso = (PTSO)ptso;
	jboolean hasData;
	jbyte* rawData;
