...
void processScenario(std::vector<ObjectiveBase*> &theObjectives,
                     ...
                     PSource sources, int numSources, void *aggrInput)
{
   Detection *det = NULL;
      ...
      if (i > 0)
      {
         ...
         if(needLinkData) {
            ...
            for (int j = 0; j < net->nlinks; j++)
            {
               ...
               double thisQuality =
                  ((fromNodeQuality > 0.0) && (links[j].flow[i] > 0.0)) ? fromNodeQuality :
                  (((toNodeQuality > 0.0) && (links[j].flow[i] < 0.0)) ? toNodeQuality : 0.0);

               if (thisQuality > 0.0)
               {
                  for (std::vector<ObjectiveBase*>::iterator iter = theObjectives.begin(); iter != theObjectives.end(); iter++)
                  {
                     (*iter)->updateImpactStatisticsDueToLink(j, thisQuality, nodes, links, i, reportStep);
                  }
               }
            }
         }
   ...
   for (int trueContaminationTime = net->nsteps - detectionDelaySteps;
         trueContaminationTime < net->nsteps; trueContaminationTime++)
   {
      std::vector<int> detNodes = det->detectionNodeList[trueContaminationTime];
      if (detNodes.size() > 0)
      {
   ...
   for (size_t k = 0; k < theObjectives.size(); k++)
   {
      ...
      {
         theObjectives[k]->finalizeForScenario(thisScenarioIndex, sources, numSources);
      }
   }
}
