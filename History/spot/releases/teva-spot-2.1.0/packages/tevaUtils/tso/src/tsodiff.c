...
int main(int argc, char **argv)
{
    ...
    maxscenario=0;
    maxscenariormse=0.;
    more1=TSO_ReadSimulationResults(tso1,net1,nodes1,sources1);
    more2=TSO_ReadSimulationResults(tso2,net2,nodes2,sources2);
    more = (more1 && more2);
    while ( more ) {
        /* Sources */
        samesource=0;
        if(net1->nsources == net2->nsources) {
            samesource=1;
            for(i=0;i<net1->nsources;i++) {
                if(strcmp(sources1[i].SourceNodeID,sources2[i].SourceNodeID)) samesource=0;
                if(tso1->fileVersion >= 2 && tso2->fileVersion >= 2) {
                    if(sources1[i].SourceStart!=sources2[i].SourceStart) samesource=0;
                    if(sources1[i].SourceStop!=sources2[i].SourceStop) samesource=0;
                    if(sources1[i].SourceStrength!=sources2[i].SourceStrength) samesource=0;
                }
                if(tso1->fileVersion >= 4 && tso2->fileVersion >= 4) {
                    if(sources1[i].SourceType!=sources2[i].SourceType) samesource=0;
                    if(sources1[i].SpeciesIndex!=sources2[i].SpeciesIndex) samesource=0;
                }
            }
        }

        if(!samesource) printf("Warning: Inconsistent source description for scenario %6d\n",scenario);
        else {
	  int printed=0;
            maxnodeidx=0;
            maxspecie=0;
			...
			if(e != 0 && (nodes2[i].c[j][k] == 0.0 || nodes1[i].c[j][k] == 0)) {
			  if(!printed)
    ...
    if (more1 || more2) {
        printf("Inconsistent ensemble size\n");
        exit(9);
    }
}
