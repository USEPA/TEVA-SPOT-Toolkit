...
double SideConstraintInstance::getOneCost(int facilityToClose, int facilityToOpen,
  ...
  if (theStatistics != "mean")
    temCost = ((double) numN) * theSolution->moveEvaluator(facilityToOpen, gamma);
  else 
    temCost = theSolution->cost(gamma)* ((double) numN)
...
bool SideConstraintInstance::findBestPair(int& bestInsert, int& bestRemove){
  double baselineCost = getCost();
  //std::cout<<" baseline cost is: "<< baselineCost<<std::endl;
  double temCost;
...
void SideConstraintInstance::descend(){
  ...
  while(findBestPair(facToInsert, facToRemove)){
    updateMeanTable(facToInsert, facToRemove);
