...
// #define EXECUTABLE /* Un-comment this line to generate an executable, i.e., a main() entry point. For both windows and linux */
...
*/
{
   ...
   if(RptFile != NULL) {
	   fclose(RptFile);
	   RptFile=NULL;
	   if(!SaveQflag) unlink(RptFile);
   }
   if(HydFile != NULL) {
	   fclose(HydFile);
	   HydFile=NULL;
	   if(!SaveHflag) unlink(HydFile);
   }
   if(OutFile != NULL) {
	   fclose(OutFile);
	   OutFile=NULL;
...
/* Check that file names are not identical */
   if (strcomp(f1,f2) || strcomp(f1,f3) || (strcomp(f2,f3) && strlen(f2) > 0))
   {
      writecon(FMT04);
      return(301);
   }
...
/* Write error message if the file cannot be opened.            */
   ...
   switch(Hydflag)
   {
/*  Changed due to problems with leftover hydraulic files
    in TEVA simulations if close wasn't called due to the sim server crashing */
      case SCRATCH:  HydFile = fopen(HYD_TMP_FILENAME, "w+b");
                     break;
