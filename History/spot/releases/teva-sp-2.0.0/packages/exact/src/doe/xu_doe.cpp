...
BitArray bits(s);
bits.reset();
for (unsigned int i=0; i<(x.size()-num); i++)
  bits.set(i);
shuffle(bits,&rng);
 ...
 * 
 */
void Bound(BasicArray<int>& bound, const BasicArray<Factor>& factors, unsigned int nruns)
{
int sumst = 0;
int sumw   = 0;
for (size_t i=0; i<factors.size(); i++) {
  unsigned int t = nruns;
  sumst += (factors[i].nlevels-1);
  sumw += factors[i].nlevels;
  bound[i] = max(t*(t*factors.size()*(factors.size()+1)/2 + t*sumst - sumw*sumw)/2,
		 t*(t*factors.size()*(factors.size()+1)/2 + t*sumst*sumst/(t-1) - sumw*sumw)/2);
  }
}
...
int AddOAColumn(BasicArray<BasicArray<int> >& x,
		 ...
		 AnyRNG& rng)
{
const unsigned int nruns = x[0].size();
if (nTry < 1) nTry=1;
...
void woa(BasicArray<Factor>& factors,
 	 unsigned int nruns,
...
for (unsigned int i=0; i<factors.size(); i++) {
  x[i].resize(factors[i].nlevels);
  xbest[i].resize(factors[i].nlevels);
  }
for (unsigned int i=0; i<nruns; i++)
  c[i].resize(nruns);
//
...
if ((argc == 1) || (params.get_parameter<bool>("help"))) {
   cout << "xu_doe [--help] <filename>" << endl;
