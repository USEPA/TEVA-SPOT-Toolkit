...
int main(int argc, char **argv)
{
  if((argc!=4)&&
     (argc!=5))
    {
      std::cout << "***Usage: sideconstraints <configuration-file> <num-samples> <random-seed> [solution-file]" << std::endl;
      return 0;
    }
  ...
  for(size_t i=0;i<theProblem.goals.size();++i)
      ...
      if((thisGoal.objective_flag==true)&&(objective==""))
	{
	  ...
	  statisticName=thisGoal.measure.front();
	  if(statisticName=="mean")
	    {
	      statistic=OBJMEAN;
	    }
	  else if(statisticName=="var")
	    {
	      statistic=OBJVAR;
	    }
	  else if(statisticName=="tce")
	    {
	      statistic=OBJTCE;
	    }
	  else if(statisticName=="worst")
	    {
	      statistic=OBJWORST;
	    }
	  else
	    {
	      std::cout << "***Unknown objective statistic specified: " << statisticName << std::endl;
	      return 0;
	    }
	}
      else 
  ...
  PMInstance *optimizationInstance(readInstance(objectiveImpactFileName.c_str(),numFacilitiesToOpen));
  if(optimizationInstance==0)
    {
  ...
  for(size_t i=0;i<sideConstraintImpactFiles.size();i++)
    {
      PMInstance *newInstance(readInstance(sideConstraintImpactFiles[i].c_str(),numFacilitiesToOpen));
      if(newInstance==0)
	{
	  std::cout << "***Failed to read problem instance" << std::endl;
	  return 0;
