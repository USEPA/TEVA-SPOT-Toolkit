				     ...
				     const std::vector<double> &detectionProbability)
{
  ...
  // detect an attack at any given time => the aggregate probability of detection/remaining
  ...
  for(std::vector<std::pair<int,double> >::const_iterator iter(thisArrivalSequence.begin());iter!=endIter;)
    {
      ...
      if((*iter).first==-1)
	{
	  cumulativeImpact=thisInstance->fastGetUndetectedImpact(theScenario);
	}
      else
	{
	  cumulativeImpact=thisInstance->fastGetDist(theScenario,(*iter).first);
	}
      ...
      while((iter!=endIter)&&(((*iter).second)==arrivalTime))
	{
      ...
      // if the attack could be detected with any non-zero probability at 
				      ...
				      const std::vector<double> &detectionProbability)
{
  ...
  for(currentIter=sensorArrivalSequence.begin();currentIter!=endIter;)
    {
      ...
      double thisImpact(thisInstance->fastGetDist(theScenario,(*currentIter).Node));
  ...
  scenarioExpectedImpact+=(thisInstance->fastGetUndetectedImpact(theScenario)*scenarioRemainingProbability);
	  ...
	  if(newFacilityArrivalTime<thisArrivalTime)
	    {
	      ...
	      thisImpact=thisInstance->fastGetDist(theScenario,newFacility);
  ...
  if(newFacilityHandled==false)
    {
      ...
      scenarioExpectedImpact+=thisInstance->fastGetDist(theScenario,newFacility)*scenarioRemainingProbability*prob;
...
{
  // for each attack scenario, scan the nodes in terms of arrival sequence to
  ...
  for(int i=1;i<=n;i++)
      ...
      for(size_t j=0;j<thisArrivalSequence.size();)
	{
	  ...
	  if(thisArrivalSequence[j].first==-1)
	    {
	      cumulativeImpact=thisInstance->undetectedImpact(i);
	    }
	  else
	    {
	      cumulativeImpact=thisInstance->fastGetDist(i,thisArrivalSequence[j].first);
	    }
			     ...
			     const std::vector<double> &detectionProbabilities)
{
  ...
  for(int i=1;i<=n;i++)
    {
      if(thisInstance->fastGetDist(i,facilityOpened)==thisInstance->fastGetUndetectedImpact(i))
	{
      ...
      else
	{
	  double val(computeScenarioExpectedImpact3(theSolution,
						    ...
						    facilityOpened,
						    thisInstance->fastGetArrivalTime(i,facilityOpened),
...
int main(int argc, char **argv)
{
  if((argc!=6)&&
     (argc!=7))
    {
      std::cout << "***Incorrect invocation - Usage is: imperfect <configuration-file> <category-definition-input-file> <location-category-input-file> <num-samples> <random-seed> [solution-file]" << std::endl;
      return 0;
    }
