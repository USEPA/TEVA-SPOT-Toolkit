...
for (size_type i=0; i<i_l_bound.size(); i++) {
  int temp = (int)floor(d_l_bound[i] + (d_u_bound[i]-d_l_bound[i]+1) * urnd());
  if (temp > i_u_bound[i])
...
unsigned int nbits = min( (unsigned int)floor((nbinary+1)*urnd()), nbinary);
binary.reset();
for (unsigned int i=0; i<nbits; i++)
  binary.set(i);
shuffle(binary,&(urnd.generator()));
}
