...
#define INCLUDE_DOT_PERCENTILES 0
...
void saveFatalityPercentiles(NamedDataRef *resultsRef, DiskCachedDataPtr fatalities, DiskCachedDataPtr sidf, int* percentiles, char *percentileDescs[], int numPercentiles);
...
void cacheData(JNIEnv *env,DiskCachedDataPtr dcd, float val, char *simID, char *injDef, void *data, int size);
...
void HIAnalysis_aggr_initialize(NamedDataRef *analysisOptionsRef, NamedDataRef *simResultsRef)
{
	...
	if(x_mem->dot)
		x_totDoseOverThresh=(int **)calloc(x_mem->dot->numThresh,sizeof(int *));
	else
...
#if INCLUDE_DOT_PERCENTILES
	if(x_mem->dot) {
		x_NumDoseOverThresh = (DiskCachedDataPtr*)calloc(x_mem->dot->numThresh,sizeof(DiskCachedDataPtr));
		for(i=0;i<x_mem->dot->numThresh;i++) {
		  char fn[80];
		  sprintf(fn,"doseover_%02d.cache",i);
		  x_NumDoseOverThresh[i]=initDiskCachedData(fn,x_mem->net->info->nnodes);
		}
	} else {
...
void cacheData(JNIEnv *env, DiskCachedDataPtr dcd, float val, char *simID, char *injDef, void *data, int size)
{
	...
	dcd->data[dcd->num]->injDef = injDef;
...
void HIAnalysis_aggr_newResults(NamedDataRef *analysisOptionsRef,NamedDataRef *intResultsRef)
{
	...
	double rank;
	float *tdata;
	int *tidata,*tdot,*binnedData;
	PHIST hist = (PHIST)calloc(1,sizeof(HIST));
	JNIEnv *env = analysisOptionsRef->env;
	int numInj;
	int log=nnodes>40000;
	char msg[256];

	x_env=env;
if(log) {
  sprintf(msg,"Entering newResults.  x_env=0x%08x",x_env);
...
if(log) {
  sprintf(msg,"newResults: 01  x_NumWorstToKeep=%d",x_NumWorstToKeep);
  ANL_UTIL_LogInfo(env,"teva.analysis.server",msg);
}
...
if(log) {
  sprintf(msg,"newResults: 02  x_PrevHist=0x%08x hist=0x%08x",x_prevHist,hist);
  ANL_UTIL_LogInfo(env,"teva.analysis.server",msg);
}
...
if(log) { ANL_UTIL_LogInfo(env,"teva.analysis.server","newResults: 03"); }
	...
	if(getString(intResultsRef,"Hist_SimID", &(tstr))==ND_FAILURE) return;
if(log) {
  sprintf(msg,"newResults: 04  Sim id = %s",tstr);
  ANL_UTIL_LogInfo(env,"teva.analysis.server",msg);
}
	...
	x_scenarioCount++;
if(log) {
  sprintf(msg,"newResults: 05  scenario count=%d",x_scenarioCount);
  ANL_UTIL_LogInfo(env,"teva.analysis.server",msg);
}

	if(getFloatArray(intResultsRef,"F_n",&tdata)==ND_FAILURE) return;
	cacheData(intResultsRef->env,x_Fatalities,hist->fh,hist->simID,hist->injDef,tdata,nnodes*sizeof(float));
	nd_free(tdata);
if(log) { ANL_UTIL_LogInfo(env,"teva.analysis.server","newResults: 06"); }
	...
	x_initFloatArray(intResultsRef,"F_t",       x_xa->f,  nsteps);
	{
		...
		cacheData(intResultsRef->env,x_sidfData,hist->fh,hist->simID,hist->injDef,xa_data,6*nsteps*sizeof(float));
		free(xa_data);
	}
if(log) { ANL_UTIL_LogInfo(env,"teva.analysis.server","newResults: 07"); }
...
if(log) { ANL_UTIL_LogInfo(env,"teva.analysis.server","newResults: 08"); }
...
if(log) { ANL_UTIL_LogInfo(env,"teva.analysis.server","newResults: 09"); }
...
if(log) { ANL_UTIL_LogInfo(env,"teva.analysis.server","newResults: 10"); }
	if(x_mem->dot) {
		if(getIntArray(intResultsRef,"TotDoseOverThreshold",&tdot)==ND_FAILURE) return;
		for(i=0;i<x_mem->dot->numThresh;i++) {
		  x_totDoseOverThresh[i]=(int *)realloc(x_totDoseOverThresh[i],x_scenarioCount*sizeof(float));
		  ...
		  cacheData(intResultsRef->env,x_NumDoseOverThresh[i],(float)x_totDoseOverThresh[i][x_scenarioCount-1],hist->simID,hist->injDef,tidata,nnodes*sizeof(float));
		  nd_free(tidata);
#endif
		}
		nd_free(tdot);
	}
if(log) { ANL_UTIL_LogInfo(env,"teva.analysis.server","newResults: 11"); }
	if(x_mem->doseBins) {
		if(getIntArray(intResultsRef,"BinnedDoses",&binnedData)==ND_FAILURE) return;
		x_BinnedDoses=(int **)realloc(x_BinnedDoses,x_scenarioCount*sizeof(int*));
		x_BinnedDoses[x_scenarioCount-1]=(int *)calloc(x_mem->doseBins->numBins,sizeof(int));
		memcpy(x_BinnedDoses[x_scenarioCount-1],binnedData,x_mem->doseBins->numBins*sizeof(int));
		nd_free(binnedData);
	}	
if(log) { ANL_UTIL_LogInfo(env,"teva.analysis.server","newResults: 12"); }
...
if(log) { ANL_UTIL_LogInfo(env,"teva.analysis.server","newResults: 13"); }
...
if(log) { ANL_UTIL_LogInfo(env,"teva.analysis.server","newResults: 14"); }
...
if(log) { ANL_UTIL_LogInfo(env,"teva.analysis.server","newResults: 15"); }
...
if(log) { ANL_UTIL_LogInfo(env,"teva.analysis.server","newResults: 16"); }
...
ANL_UTIL_LogInfo(env,"teva.analysis.server","getResults: 01");
	...
	if(addVariable(resultsRef,"Hist_InjID",     STRING_ARRAY_TYPE)==ND_FAILURE) return;
	...
	if(x_mem->dot) {
		...
		for(i=0;i<x_mem->dot->numThresh;i++) {
		  dot[i]=(int*)calloc(x_mem->net->info->nnodes,sizeof(int));
		}
	} else {
		dot=NULL;
	}
...
//			printf("%f %f\n",tHist->fh,totalFatalities[i]); fflush(stdout);
		}
	}
	...
	if(x_mem->dot) {
		...
		for(i=0;i<numDThresh;i++) {
		  char varName[80];
		  sprintf(varName,"Map_DoseOverThreshold_%02d",i);
		  if(setIntArray(resultsRef,varName,  dot[i],  x_mem->net->info->nnodes)==ND_FAILURE) return;
		  free(dot[i]);
		}
		for(i=0;i<numRThresh;i++) {
		  char varName[80];
		  sprintf(varName,"Map_ResponseOverThreshold_%02d",i);
		  if(setIntArray(resultsRef,varName,  dot[i+numDThresh],  x_mem->net->info->nnodes)==ND_FAILURE) return;
	...
	if(setStringArray(resultsRef,"Hist_InjID", sData,  x_scenarioCount)==ND_FAILURE) return;
	free(sData);
...
//	if(getIntArray(analysisOptionsRef, "Percentiles", &percentiles)==ND_FAILURE) return;

	{
		...
		int *tival = (int*)calloc(numPercentiles+1,sizeof(int));
		...
		if(x_mem->dot) {
			for(i=0;i<numDThresh;i++) {
			  char varName[80];
			  sprintf(varName,"TotalDoseOverThresholdSummary_%02d",i);
			  computeIntegerSummary(x_totDoseOverThresh[i],x_scenarioCount,percentiles,tival,numPercentiles);
			  if(setIntArray(resultsRef,varName, tival,numPercentiles+1)==ND_FAILURE) return;
			}
			for(i=0;i<numRThresh;i++) {
			  char varName[80];
			  sprintf(varName,"TotalResponseOverThresholdSummary_%02d",i);
			  computeIntegerSummary(x_totDoseOverThresh[i+numDThresh],x_scenarioCount,percentiles,tival,numPercentiles);
			  if(setIntArray(resultsRef,varName, tival,numPercentiles+1)==ND_FAILURE) return;
			}
		}
		free(tval);
		...
		if(x_mem->dot) {
			for(i=0;i<x_mem->dot->numThresh;i++) {
				qsort(x_NumDoseOverThresh[i]->data,x_NumDoseOverThresh[i]->num,sizeof(DiskCachedDataPtr),sortCachedDataAsc);
			}
		}
#endif
...
ANL_UTIL_LogInfo(env,"teva.analysis.server","getResults: 21");
		saveFatalityPercentiles(resultsRef,x_Fatalities,x_sidfData,percentiles,rowNames,numPercentiles);
ANL_UTIL_LogInfo(env,"teva.analysis.server","getResults: 22");
#if INCLUDE_DOT_PERCENTILES
		if(x_mem->dot) {
...
void saveTotalDoseOverThreshPercentiles(NamedDataRef *resultsRef, DiskCachedDataPtr *totDoseOverThreshBySrc, int numThresh, int* percentiles, char *percentileDescs[], int numPercentiles)
{
	...
	char msg[256];
	JNIEnv *env=resultsRef->env;
	sprintf(msg,"saveTotalDoseOverThreshPercentiles 01");
	ANL_UTIL_LogInfo(env,"teva.analysis.server",msg);
...
void saveFatalityPercentiles(NamedDataRef *resultsRef, DiskCachedDataPtr fatalitiesBySrc, DiskCachedDataPtr sidf, int* percentiles, char *percentileDescs[], int numPercentiles)
{
	int i;
...
int writeHIADataFile(NamedDataRef *analysisOptionsRef,NamedDataRef *resultsRef, const char *dir, char *fn) {
	...
	for(i=0;i<nscenarios;i++) {
	  ...
	  fprintf(fp,"%s\t%f\t%f\t%f\t%f\t%d\t%f\t%f\n",
		  simids[i],maxConc[i],maxDose[i],totInf[i],totFatal[i],nFatalNodes[i],tda[i],tra[i]);
	}
...
int writeDoseOverThreshold(NamedDataRef *analysisOptionsRef,NamedDataRef *resultsRef, const char *dir, char *fn) {
	char fname[256];
	FILE *fp;
	char msg[256];
	char varName[256];
	...
	sprintf(fname,"%s/HIA_DoseOverThreshold.txt",dir);
	fp = fopen(fname,"w");
	if(fn[0]!='\0')
	  strcat(fn,",");
	strcat(fn,"HIA_DoseOverThreshold.txt");
	fprintf(fp,"ID\tResponse\tDose=>");
	for(i=0;i<numDThresh;i++) {
		float dose;
		sprintf(varName,"DoseThreshold_%02d",i);
		if(getFloat(resultsRef,varName,&dose)==ND_FAILURE) return;
		fprintf(fp,"\t%g",dose);
	}
	fprintf(fp,"\n");
	fprintf(fp,"\t\tResponse=>");
...
int writeBinnedDoses(NamedDataRef *analysisOptionsRef,NamedDataRef *resultsRef, const char *dir, char *fn) {
	...
	if(getInt(resultsRef,"NumScenarios",&nscenarios)==ND_FAILURE) return 0;
	if(getInt(resultsRef,"BDNumBins",&numBins)==ND_FAILURE) return;
	if(numBins==0) return 1;
	if(getStringArray(resultsRef,"Hist_SimID",&simids)==ND_FAILURE) return 0;
	binnedDoses=(int **)calloc(nscenarios,sizeof(int **));
	for(i=0;i<nscenarios;i++ ) {
		if(getIntArrayAtIndex(resultsRef,"BinnedDoses",i,&binnedDoses[i])==ND_FAILURE) return;
	}
	if(getFloatArray(resultsRef,"BDMinDoses",&minD)==ND_FAILURE) return;
	if(getFloatArray(resultsRef,"BDMaxDoses",&maxD)==ND_FAILURE) return;
	if(getFloatArray(resultsRef,"BDMinResponses",&minR)==ND_FAILURE) return;
	if(getFloatArray(resultsRef,"BDMaxResponses",&maxR)==ND_FAILURE) return;
...
int writeWorstCaseFiles(NamedDataRef *analysisOptionsRef,NamedDataRef *resultsRef, const char *dir, char *fn) {
	char fname[256];
	FILE *fp;
	char msg[256];
	JNIEnv *env=resultsRef->env;
	int numWorstToKeep,i,nt;

	if(getInt(analysisOptionsRef, "NumWorstCaseToKeep", &numWorstToKeep)==ND_FAILURE) return 0;

	if(numWorstToKeep>0) {
		int nscenarios,nnodes;
		char **nodeIDs;
		float *pop;
		// write out individual files for worst scenarios
		if(getInt(resultsRef,"NumWorstScenarios",&nscenarios)==ND_FAILURE) return 0;
		...
		for(i=0;i<nscenarios;i++) {
			...
			sprintf(name,"SimID[%02d]",i);
			if(getString(resultsRef,name, &simID) ==ND_FAILURE) return 0;
			fprintf(fp,"Simulation ID\t%s\n",simID);
			nd_free(simID);

			sprintf(name,"RankValue[%02d]",i);
			if(getDouble(resultsRef,name, &totalConcentration) ==ND_FAILURE) return 0;
			fprintf(fp,"Total Fatalities\t%lf\n",totalConcentration);

			sprintf(name,"ReceptorDosage[%02d]",i);
			if(getFloatArray(resultsRef,name, &dos) ==ND_FAILURE) return 0;
			sprintf(name,"ReceptorResponse[%02d]",i);
			if(getFloatArray(resultsRef,name, &res) ==ND_FAILURE) return 0;
			sprintf(name,"ReceptorS[%02d]",i);
			if(getFloatArray(resultsRef,name, &s) ==ND_FAILURE) return 0;
			sprintf(name,"ReceptorI[%02d]",i);
			if(getFloatArray(resultsRef,name, &ii) ==ND_FAILURE) return 0;
			sprintf(name,"ReceptorD[%02d]",i);
			if(getFloatArray(resultsRef,name, &d) ==ND_FAILURE) return 0;
			sprintf(name,"ReceptorF[%02d]",i);
			if(getFloatArray(resultsRef,name, &f) ==ND_FAILURE) return 0;
...
void HIAnalysis_aggr_writeResults(NamedDataRef *analysisOptionsRef,NamedDataRef *resultsRef, const char *dir, char *fn) {
	...
	if(!writeDoseOverThreshold(analysisOptionsRef,resultsRef,dir,fn)) return;
	...
	if(!writeWorstCaseFiles(analysisOptionsRef,resultsRef,dir,fn)) return;
...
#if INCLUDE_DOT_PERCENTILES
			freeDiskCachedData(analysisOptionsRef->env,&x_NumDoseOverThresh[i]);
#endif
		}
