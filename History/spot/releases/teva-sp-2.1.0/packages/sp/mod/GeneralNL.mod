...
minimize meanImpact:
	sum{a in Attacks[objectiveGoal]} 
		AttackWeight[objectiveGoal,a] 
		* 
	   	sum{L in AttackSuperLocations[objectiveGoal,a]}
		  (
...
subject to compute_first_probability{g in ActiveGoals, a in Attacks[g],
		  asl in AttackSuperLocations[g,a],
