...
static char *usageString="usage: tso2text directoryOrFile [patternIfDirectory] [-sStorageMethod -fFileVersion] [-ident STRING] [-n numToWrite | -ids [id1 id2 ... idn]\n";
...
int main(int argc, char **argv)
{
	...
	while(n <= numToWrite && TSO_ReadSimulationResults(tso,net,nodes,sources)>0) {
		...
		if(ids == NULL || needToWriteID(sources[0].SourceNodeID,ids,numToWrite)) {
			...
			sprintf(fn,"%s%s%s_%s.txt",outputFileBase,ident==NULL?"":"_",ident==NULL?"":ident,srcID);
			fp = fopen(fn,"w");
