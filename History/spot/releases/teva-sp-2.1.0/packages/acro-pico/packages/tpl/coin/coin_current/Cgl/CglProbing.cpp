                                   ...
                                   const CglTreeInfo info) const
{
  ...
  double cutoff;
  // CAP: changed because with solvers like soplex that, at least at
  // the time of this fix, don't allow a dual cutoff, this value could
  // be garbage
  ...
  if (feasible&&PROBING_EXTRA_STUFF) {
    ...
    double cutoff;
  // CAP: changed because with solvers like soplex that don't
  // allow a dual cutoff, this value could be garbage
  bool cutoff_OK = si.getDblParam(OsiDualObjectiveLimit,cutoff);
  if (!cutoff_OK)  // cut off isn't set or isn't valid
    cutoff = si.getInfinity();
    cutoff *= direction;
