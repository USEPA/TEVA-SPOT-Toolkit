        ...
        if (gap<1.0e10&&isProvenOptimal()) {
          ...
          for (i=0;i<numberIntegers;i++) {
            ...
            if (upper[iColumn]>lower[iColumn]) {
              ...
              if (basis_.getStructStatus(iColumn)==CoinWarmStartBasis::atLowerBound&&
                  djValue>gap) {
                nFixed0++;
                setColUpper(iColumn,lower[iColumn]);
              } else if (basis_.getStructStatus(iColumn)==CoinWarmStartBasis::atUpperBound&&
                         -djValue>gap) {
                nFixed1++;
