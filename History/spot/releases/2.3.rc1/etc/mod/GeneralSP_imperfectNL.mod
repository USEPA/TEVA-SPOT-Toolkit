...
subject to pickFirstSensor {g in ActiveGoals, a in Events[g]}:
        sum {L in EventSuperLocations[g,a]} WitnessProbability[g,a,L] = 1;
...
## Worst Goal Value
##
subject to worstGoals {(g,m) in ActiveGMPair diff {("ns","total"), ("cost","total")}, a in Events[g] : m == "worst"}:
	sum{L in EventSuperLocations[g,a]} GoalMeasurement[g,a,WorstSuperLocationMember[g,a,L]]*WitnessProbability[g,a,L] <= GoalValue[g,m];
...
##			i witnesses event a is the probability that all
...
##########################################################################
subject to compute_first_witness_probability{g in ActiveGoals, a in Events[g],
		  asl in EventSuperLocations[g,a],
...
subject to compute_witness_probabilities{g in ActiveGoals, a in Events[g],
					  asl in EventSuperLocations[g,a]}:
	ProbabilityOfPlaying[g,a,asl] = 
