...
class Repository(object):
    ...
    def easy_install(self, setup, dir):
        print self.easy_install_path, setup, os.path.exists(dir)
        if self.easy_install_path is None:
            return
        if setup:
            if not os.path.exists(dir+os.sep+self.pypi):
                self.run([self.easy_install_path+executable_extension, '-q', self.pypi])
            else:
                self.run([self.python, 'setup.py', 'install'], dir=dir)
        else:
            self.run([self.easy_install_path+executable_extension, '-q', '--editable', '--build-directory', '.', self.pypi], dir=os.path.dirname(dir))

    def easy_upgrade(self):
        self.run([self.easy_install_path+executable_extension, '-q', '--upgrade', self.pypi])
