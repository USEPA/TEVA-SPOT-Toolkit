...
{     int i;
      ...
      dsa->y = y;
      dsa->z = z;
      dsa->D = ucalloc(1+n, sizeof(double));
      /* P := I */
      dsa->P = ucalloc(1+m+m, sizeof(int));
      for (i = 1; i <= m; i++) dsa->P[i] = dsa->P[m+i] = i;
      /* S := A*A', symbolically */
      dsa->S_ptr = ucalloc(1+m+1, sizeof(int));
      ...
      dsa->rmu = 0.0;
      dsa->rmu0 = 0.0;
      dsa->phi_min = ucalloc(1+ITER_MAX, sizeof(double));
      dsa->best_iter = 0;
      dsa->best_x = ucalloc(1+n, sizeof(double));
      dsa->best_y = ucalloc(1+m, sizeof(double));
      dsa->best_z = ucalloc(1+n, sizeof(double));
      dsa->best_obj = 0.0;
      dsa->dx_aff = ucalloc(1+n, sizeof(double));
      dsa->dy_aff = ucalloc(1+m, sizeof(double));
      dsa->dz_aff = ucalloc(1+n, sizeof(double));
      dsa->alfa_aff_p = 0.0;
      dsa->alfa_aff_d = 0.0;
      dsa->mu_aff = 0.0;
      dsa->sigma = 0.0;
      dsa->dx_cc = ucalloc(1+n, sizeof(double));
      dsa->dy_cc = ucalloc(1+m, sizeof(double));
      dsa->dz_cc = ucalloc(1+n, sizeof(double));
      dsa->dx = dsa->dx_aff;
      dsa->dy = dsa->dy_aff;
      dsa->dz = dsa->dz_aff;
      dsa->alfa_max_p = 0.0;
...
{     int m = dsa->m;
      ...
      p = ucalloc(1+m, sizeof(double));
      q = ucalloc(1+n, sizeof(double));
      r = ucalloc(1+n, sizeof(double));
      /* p = b - A*x */
      A_by_vec(dsa, x, p);
