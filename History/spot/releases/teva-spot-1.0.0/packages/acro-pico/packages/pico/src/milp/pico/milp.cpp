...
if (lp_solve) {
   ...
   if (solvedState == PicoLPInterface::unbounded) {
      ...
      ucout << "\tCPU RunTime=         " << run_time << endl;
      ucout << "\tCPU TotalTime=       " << (CPUSeconds() - startTime) << endl;
      ucout << "\tWallClock TotalTime= " << (WallClockSeconds() - startWall) << endl;
...
void MILP::serialPrintSolValue(std::ostream& stream)
{
  if (problemIsFeasible()) {
    ...
    {
    if (abortReason)
      stream << endl << "Aborted without a feasible solution\n";
    else stream << endl << "Integer Infeasible";
    }
  stream << endl << endl;
}
