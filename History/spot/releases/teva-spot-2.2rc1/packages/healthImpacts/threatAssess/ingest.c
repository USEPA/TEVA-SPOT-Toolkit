...
void ingestTimes(int pop, IndividualIngestionDataPtr *idp[], float stepsize, SplineDataPtr *invcdf, int times, int volMode);
...
void initializeIngestionData(IngestionDataPtr p, PNet net, PNode nodes, PDR dr, PopulationDataPtr pd)
{
	...
	for(i=0;i<p->numNodes;i++) {
		int pi;
		ingestTimes((int)nodes[i].pop,&(p->ingestionData[i]),net->info->stepsize,p->fTiming,p->timingMode,p->volumeMode);
	...
	if(times==IMFixed5 || times==IMFixed2) {
		...
		*idp = (IndividualIngestionDataPtr*)calloc(tPop,sizeof(IndividualIngestionDataPtr));
		for(i=0;i<tPop;i++) {
			int j;
			IndividualIngestionDataPtr p = (IndividualIngestionDataPtr)calloc(1,sizeof(IndividualIngestionData));
			for(j=0;j<2;j++) p->times[j] /= stepsize;
		...
		*idp = (IndividualIngestionDataPtr*)calloc(pop,sizeof(IndividualIngestionDataPtr));
		for(i=0;i<pop;i++) {
			...
			(*idp)[i]=p;
			for(j=0;j<5;j++) {
				p->times[j] = (((int)(runif()*240))/10.0)/stepsize;
			}
		...
		*idp = (IndividualIngestionDataPtr*)calloc(pop,sizeof(IndividualIngestionDataPtr));
		for(i=0;i<pop;i++) {
			...
			for(j=0;j<5;j++) p->times[j] = (int)(p->times[j]/stepsize);
			needToSort=0;
			for(j=4;j>=0;j--) {
				if(p->times[j] >= 24) {
...
void freeIngestionData(IngestionDataPtr idp, PNode nodes) {
	int i,j,nodePop;
	float **nd;
