...
/*	net->fltmax=fltmax; */
	...
	net->info->qualResults->nodeC=(float ***)calloc(1,sizeof(float **));
	net->info->qualResults->linkC=(float ***)calloc(1,sizeof(float **));
	net->info->qualResults->nodeC[0]=(float **)calloc(nnodes,sizeof(float*));
	for(i=0;i<nnodes;i++) {
		net->info->qualResults->nodeC[0][i]=(float *)calloc(net->info->numSteps,sizeof(float));
	}
	...
	}
	{
		...
		if(hasData(simResultsRef,"demands",FLOAT_ARRAY_2D_TYPE,&hasDemand)==ND_FAILURE) {
			...
			if(hasFlow) {
				...
				for(j=0;j<net->info->numSteps;j++) {
					f[j]=(float*)calloc(nnodes,sizeof(float));
				}
			...
			if(hasVel) {
				...
				for(j=0;j<net->info->numSteps;j++) {
					v[j]=(float*)calloc(nnodes,sizeof(float));
				}
				for(j=0;j<nlinks;j++) {
					v[i][j] = tres[j];
				}
...
//ANL_UTIL_LogInfo(env,"teva.analysis.server","freeHIAMemory: 01");
...

	{
		...
		for(i=0;i<mem->net->info->numSpecies;i++) {
			int n;
			for(n=0;n<nnodes;n++) {
				if(qd->linkC != NULL) free(qd->linkC[i][n]);
				if(qd->nodeC != NULL) free(qd->nodeC[i][n]);
			}
			if(qd->linkC != NULL) free(qd->linkC[i]);
			if(qd->nodeC != NULL) free(qd->nodeC[i]);
		}
