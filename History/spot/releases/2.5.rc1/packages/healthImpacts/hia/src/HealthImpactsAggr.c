...
void HIAnalysis_aggr_initialize(NamedDataRef *analysisOptionsRef, NamedDataRef *simResultsRef)
{
	...
	nsteps = x_mem->net->info->numSteps+x_mem->dr->nL+x_mem->dr->nF;
	x_scenarioCount=0;
	...
	x_Fatalities = initDiskCachedData("fatalities.cache",x_mem->net->info->numNodes,x_numScenarios);
#if INCLUDE_DOT_PERCENTILES
	if(x_mem->dot) {
		int i;
		...
		for(i=0;i<x_mem->dot->numThresh;i++) {
		  ...
		  x_NumDoseOverThresh[i]=initDiskCachedData(fn,x_mem->net->info->numNodes,x_numScenarios);
		  x_totDoseOverThresh[i]=(int *)x_calloc(x_numScenarios,sizeof(int)," x_totDoseOverThresh[i]");
		}
	} else {
...
void cdpFree(const void *data)
{
	...
	free(cdp->injDef);
	free(cdp->conc);
	freeXA(&cdp->na,x_mem->net->info->numNodes);
	free(cdp);
}
