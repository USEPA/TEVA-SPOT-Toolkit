		     ...
		     bool scenario_aggr)
{
  ...
  // a sensor at the source index can always immediately detect the event.
  ...
  for(int i=0;i<net->nsources;i++)
    {
      ...
      int nodeID = sources[i].SourceNodeIndex-1;
      int startTimeInMinutes = (long) (sources[i].SourceStart / 60.0);
      long detTimeInSteps = (long) (startTimeInMinutes/reportStep);
      if (detTimeInSteps >= (net->nsteps-1)){
        detTimeInSteps = net->nsteps - 1;
      }
      trueNodeDetections[nodeID] = (int) (startTimeInMinutes/reportStep);
      effectiveNodeDetections[detTimeInSteps+detectionDelaySteps].push_back
						(nodeID); 
    }
