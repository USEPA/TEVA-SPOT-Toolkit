...
void SPProblem::read_config_file(istream& istr, bool loadImpacts, bool loadWtime)
{
  int ngoals;
  string fname;
  ...
  while (curr != end) {
	...
	if (loadImpacts) {
	    ...
	    if (loadWtime)
	       read_impacts(ifstr, curr->impacts, NULL, delay, &(curr->wtime), feasible_location, reduced_repn_map);
	    else
	       read_impacts(ifstr, curr->impacts, NULL, delay, NULL, feasible_location, reduced_repn_map);
	}
	if (conserve_memory)
  ...
  //
  // If we've loaded the impacts, then we don't need this info any more.  In that case, we're generating
...
double SPGoalInfo::get_impact(int event, int loc) const
{
  if ((event >= 0) || (event < num_events())){
    const std::map<int, double> &impactmap = impacts[event];
    std::map<int, double>::const_iterator curr = impactmap.find(loc);
    if (curr != impactmap.end()){
      return curr->second;
    }
  }
  
  std::cerr << "Error in SPGoalInfo::get_impact(";
  std::cerr << event << ", " << loc << ")" << std::endl;
  exit(1);
}

void SPGoalInfo::make_witness_time_map(int event, std::map<int,int> &tmap) const
{
  tmap.erase(tmap.begin(),tmap.end());
  if ((event >= 0) || (event < num_events())){
    const std::list<std::pair<int,int > > &tm = wtime[event];
    std::list<std::pair<int,int > >::const_iterator curr = tm.begin();
    while (curr != tm.end()){
      tmap[curr->first] = curr->second;
      curr++;
    }
  }
  else{
    std::cerr << "Error in SPGoalInfo::make_witness_time_map(";
    std::cerr << event << ")" << std::endl;
    exit(1);
  }
}

int SPGoalInfo::get_witness_time(int loc, std::map<int, int> &timemap) const
{
  std::map<int, int>::iterator curr = timemap.find(loc);
  if (curr != timemap.end()){
    return curr->second;
  }

  std::cerr << "Error in SPGoalInfo::get_witness_time(";
                                        ...
                                        bool output_impact)
{
  ...
  utilib::BasicArray<map<int,std::list<std::pair<int,int> > > >::iterator sltimecurr = sl_wtime.begin();

  std::map<int, int> time_lookup;
  int eventNum=0;
  ...
  while (curr != end) {
    ...
    if (output_impact){
      make_witness_time_map(eventNum, time_lookup); // need earliest witness
    }
	...
	continue;
      ...
      if ((i<ord.size()) && (values[ord[last]] < values[ord[i]])) 
        cerr << "ERROR: somehow our items are not sorted!";
	...
	if (lcurr == membersets.end()) {
	  ...
	  ndx = membersets.size();
        ...
        if (!output_impact){
          ...
          int first_witness_time = get_witness_time(first_witness, time_lookup);
          double first_witness_impact = get_impact(eventNum, first_witness);

          for (int loc=1; loc < locs.size(); loc++){
            int witness = locs[loc];
            int witness_time = get_witness_time(witness, time_lookup);
          ...
          std::pair<int, int> sl(ndx, first_witness);
          std::map<int, std::list<std::pair<int,int> > >::iterator timerec = 
            sltimecurr->find(first_witness_time);
          if (timerec == sltimecurr->end()){
            std::list<std::pair<int, int> > L(1,sl);
    ...
    if (output_impact){
      ...
      while (ncurr != fixed_nodes.end()){
        ...
        int witness_time = get_witness_time(fixed_node, time_lookup);
        std::pair<int, int> sl(ndx, fixed_node);
        std::map<int, std::list<std::pair<int,int> > >::iterator timerec = 
      ...
      time_lookup.erase(time_lookup.begin(),time_lookup.end());
      sltimecurr++;
    }
  ...
  if (output_impact){
    wtime.clear();    // don't need this
  }
}



void SPProblem::write_impact_file(ostream& os, int goal_index) const
{
const SPGoalInfo& goal = goals[goal_index];
os << num_nodes << endl;
os << "1 " << delay << endl;
int i=0;
std::vector<std::list<std::pair<int,int> > >::const_iterator curr = goal.wtime.begin();
std::vector<std::list<std::pair<int,int> > >::const_iterator end  = goal.wtime.end();
while (curr != end) {
  std::list<std::pair<int,int> >::const_iterator mcurr = curr->begin();
  std::list<std::pair<int,int> >::const_iterator mend  = curr->end();
  while (mcurr != mend) {
    std::map<int,double>::const_iterator icurr = (goal.impacts[i]).find(mcurr->first);
...
void SPProblem::write_aggregated_impacts(std::ostream& osIm, int goal_index) const
{
  const SPGoalInfo& goal = goals[goal_index];
  ...
  for (int event=0; event < numevents; event++){
    ...
    std::map<int, std::list<std::pair<int, int> > > timemap = 
      goal.sl_wtime[event];
     
    std::map<int, std::list<std::pair<int, int> > >::iterator tcurr = 
      timemap.begin();
    std::map<int, std::list<std::pair<int, int> > >::iterator tend = 
      timemap.end();
    ...
    while (curr != wtimes.end()){
      ...
      std::map<int, std::list<std::pair<int, int> > > currmap = 
        goal.sl_wtime[event];

      std::map<int, std::list<std::pair<int, int> > >::iterator currlist = 
        currmap.find(wtime);

      std::list<std::pair<int, int > > sllist = currlist->second;
      std::list<std::pair<int, int > >::iterator slcurr = sllist.begin();
      while (slcurr != sllist.end()){
        int superloc = slcurr->first;
        int origloc = slcurr->second;   // earliest witness
        double impact = goal.get_impact(event, origloc);
...
void SPProblem::write_aggregated_config_file(std::ostream& ostr, std::vector<std::string> &impactFiles)
{
  ...
  while (curr != end){
    utilib::BasicArray<std::map<int,std::list< std::pair<int, int> > > >
      timemaps = curr->sl_wtime;

    utilib::BasicArray<std::map<int,std::list< std::pair<int, int> > > >::iterator 
    ...
    int event = 1;
    while (evcurr != evend){
      std::map<int,std::list< std::pair<int, int> > >::iterator timecurr = 
        (*evcurr).begin();
      std::map<int,std::list< std::pair<int, int> > >::iterator timeend = 
        (*evcurr).end();

      while (timecurr != timeend){
        std::list<std::pair<int, int> > sllist = timecurr->second;
        std::list<std::pair<int, int> >::iterator slcurr = sllist.begin(); 
        std::list<std::pair<int, int> >::iterator slend = sllist.end(); 
        while(slcurr != slend){
          int slID = slcurr->first;
          if (slMap[slID].front() == -2){
            int redReprID = slcurr->second;
            int fixed = 0;
            int origID = -1;
            if (redReprID > 0){
...
*/
      ...
      int num_impacts;
      for (unsigned int i=0; i<curr->impacts.size(); i++) {
	total_impacts += curr->impacts[i].size();
      }
	...
	*outputfile_name = outputfile_name->substr(last_slash+1,outputfile_name->length()-1);
      ...
      *os << num_nodes << " " << curr->impacts.size() << " " << total_impacts << " " << endl;
      num_impacts = curr->impacts.size();
      for (unsigned int i=0; i<num_impacts; i++) {
        map<int,double>::iterator mcurr = curr->impacts[i].begin();
        map<int,double>::iterator mend  = curr->impacts[i].end();
