...
DiskCachedDataPtr initDiskCachedData(char *fn, int nelems,int numScenarios);
...
void HIAnalysis_aggr_initialize(NamedDataRef *analysisOptionsRef, NamedDataRef *simResultsRef)
{
	...
	x_Fatalities = initDiskCachedData("fatalities.cache",x_mem->net->info->nnodes,x_numScenarios);
#if INCLUDE_DOT_PERCENTILES
	if(x_mem->dot) {
		int i;
		...
		for(i=0;i<x_mem->dot->numThresh;i++) {
		  ...
		  x_NumDoseOverThresh[i]=initDiskCachedData(fn,x_mem->net->info->nnodes,x_numScenarios);
		  x_totDoseOverThresh[i]=(int *)x_calloc(x_numScenarios,sizeof(int)," x_totDoseOverThresh[i]");
		}
	} else {
...
#endif
	x_sidfData = initDiskCachedData("sidf.cache",nsteps,x_numScenarios);
...
{
	int i;
	DiskCachedDataPtr p = (DiskCachedDataPtr)x_calloc(1,sizeof(DiskCachedData),"p (initDiskCachedData)");
	strcpy(p->fn,fn);
	p->fp=fopen(fn,"w+b");
	p->num=0;
	p->size=nelems;
	p->data=(CachedDataPtr*)x_calloc(numScenarios,sizeof(CachedDataPtr),"dcd->data");
	for(i=0;i<numScenarios;i++) {
	  p->data[i] = (CachedDataPtr)x_calloc(1,sizeof(CachedData),"dcd->data[dcd->num]");
	}
	return p;
}
...
void cacheData(JNIEnv *env, DiskCachedDataPtr dcd, float val, char *simID, char *injDef, void *data, int size)
{
	...
	char type[32];
	strcpy(type,"unknown");
	if(dcd==x_Fatalities)    strcpy(type,"fatalities");
	else if(dcd==x_sidfData) strcpy(type,"sidf_data");
#if INCLUDE_DOT_PERCENTILES
	else {
	  if(x_mem->dot) {
	    int i;
	    for(i=0;i<x_mem->dot->numThresh;i++) {
	      if (dcd==x_NumDoseOverThresh[i]) sprintf(type,"dot_%02d",i);
	    }
	  }
	}
#endif
...
void HIAnalysis_aggr_newResults(NamedDataRef *analysisOptionsRef,NamedDataRef *intResultsRef)
{
	...
	if(x_NumWorstToKeep>0) {
		...
		if(rank>x_nthWorstScenario) {
			ConcentrationDataPtr cdp = (ConcentrationDataPtr)x_calloc(1,sizeof(ConcentrationData),"cdp");
			...
			cdp->simID=(char*)x_calloc(strlen(tstr)+1,sizeof(char),"cdp->simID");
			strcpy(cdp->simID,tstr);
			nd_free(tstr);
			if(getString(intResultsRef,"Hist_InjDef", &(tstr))==ND_FAILURE) return;
			cdp->injDef=(char*)x_calloc(strlen(tstr)+1,sizeof(char),"cdp->injDef");
			strcpy(cdp->injDef,tstr);
			nd_free(tstr);
...
int writeHIADataFile(NamedDataRef *analysisOptionsRef,NamedDataRef *resultsRef, const char *dir, char *fn) {
	...
	for(i=0;i<nscenarios;i++) {
	  fprintf(fp,"%s %s\t%f\t%f\t%f\t%f\t%d\t%f\t%f\n",
		  simids[i],injdefs[i],maxConc[i],maxDose[i],totInf[i],totFatal[i],nFatalNodes[i],tda[i],tra[i]);
	}
...
//	char msg[256];
	...
	fprintf(fp,"\n");
	for(i=0;i<nscenarios;i++) {
		fprintf(fp,"%s %s\t%f\t",simids[i],injdefs[i],tra[i]);
...
int writeBinnedDoses(NamedDataRef *analysisOptionsRef,NamedDataRef *resultsRef, const char *dir, char *fn) {
	...
	for(s=0;s<nscenarios;s++) {
		fprintf(fp,"%s %s\t",simids[s],injdefs[s]);
		for(i=0;i<numBins;i++) {
...
int writeWorstCaseFiles(NamedDataRef *analysisOptionsRef,NamedDataRef *resultsRef, const char *dir, char *fn) {
	...
	if(numWorstToKeep>0) {
		...
		for(i=0;i<nscenarios;i++) {
			...
			sprintf(name,"InjDef[%02d]",i);
			if(getString(resultsRef,name, &injDef) ==ND_FAILURE) return 0;
			fprintf(fp,"Simulation ID\t%s %s\n",simID,injDef);
			nd_free(simID);
			nd_free(injDef);
...
float **getNodeConcentrations(void *ref,char *name,int nsteps,int nnodes) {
	int i;
	float **conc = (float**)x_calloc(nsteps,sizeof(float*),"conc");
	for(i=0;i<nsteps;i++) {
		float *tres;
		conc[i] = (float*)x_calloc(nnodes,sizeof(float),"conc[i]");
