...
void getSourceDescription(int nsources,PSource sources, PTSO tso, char *sDesc);
...
int main(int argc, char **argv)
{
    ...
    maxscenariormse=0.;
    id1=0;
    scens1=(int*)calloc(tso1->count,sizeof(int));
    scens2=(int*)calloc(tso2->count,sizeof(int));
	...
	while ( id1 < tso1->count) {
		...
		if(loaded1 && loaded2) {
			...
			if(samesource!=SOURCE_SAME) {
				...
				for(j=0;j<net1->nbulkspecies;j++) {
					for(i=0;i<net1->nnodes;i++) {
						...
						for(k=0;k<net1->nsteps;k++) {
							if(nodes1[i].c[j][k]>0 || nodes2[i].c[j][k]>0) gt0++;
							e=(float)(nodes1[i].c[j][k]-nodes2[i].c[j][k]);
							if(fabs(e)> 1e-10 && (nodes2[i].c[j][k] == 0.0 || nodes1[i].c[j][k] == 0)) {
								if(!printed)
				...
				getSourceDescription(net1->nsources, sources1, tso1, scenDesc);
...
//      	ps = &idx->source[0];
//      	printf("Source 2: %8s %8d %8d %8.3f %d\n",ps->SourceNodeID,ps->SourceStart,ps->SourceStop,ps->SourceStrength,ps->SourceType);
      	return i;
      }
    }
  }
...
void getSourceDescription(int nsources,PSource sources, PTSO tso, char *sDesc)
{
	...
	for(i=0;i<nsources;i++) {
		...
		if(tso->fileVersion >= 2) {
			...
			sprintf(tstr,fmtStr,sources[i].SourceNodeID);
		}
		strcat(sDesc,tstr);
	}
}
