...
public:
  ...
  ///
  virtual void write_parameters(ostream& os, bool verbose=true,
					const char* seperator="\n") const;

  ///
  virtual void write_parameters_html(ostream& os) const;
  ...
  virtual void write_parameter_values(ostream& os, 
				const char* opt_label="") const;

  ///
  virtual void read_parameter_values(istream& is, const string& terminator);

  ///
  virtual void read_parameter_values(istream& is)
  ...
  bool parameter_exists(const string& name)
	{
#if !defined(COUGAR)
	map<string,int>::iterator curr = parameters.find(name);
	if (curr == parameters.end())
   	   return false;
	return true;
#else
        list<pair<string,int> >::iterator curr = parameters.begin();
        list<pair<string,int> >::iterator end  = parameters.end();
	while (curr != end) {
          if ((*curr).first == name)
             return true;
          curr++;
  ...
  ///
  template <class T>
  void create_parameter(const string& name, T& data, const string& syntax,
		const string& default_value, const string& description)
		{create_categorized_parameter(name,data,syntax,default_value,
			description,"General");}

  ///
  template <class T>
  void create_categorized_parameter(const string& name, T& data, 
		const string& syntax,
		const string& default_value, const string& description,
  ...
  void create_categorized_parameter(const string& name, T& data, 
		const string& syntax,
		const string& default_value, const string& description,
		const string& category, FuncT validation_func);

  ///
  void set_parameter_callback(const string& name, 
		std::unary_function<AnyReference,void>& func);

  ///
  void set_parameter_default(const string& name, const string& default_value);

  ///
  Parameter& get_parameter_object(const string& name);
...
protected:
  ...
  ///
  void add_parameter(const Parameter& param, const string& category)
	{
  ...
  ///
  void add_parameter(Parameter* param, const string& category,
							bool ownership=true);

  ///
  int parameter_index(const string& name) const
	{
#if !defined(COUGAR)
	map<string,int>::const_iterator curr = parameters.find(name);
	if (curr == parameters.end())
   	   throw invalid_argument("missing parameter");
	return (*curr).second;
#else
        list<pair<string,int> >::const_iterator curr = parameters.begin();
        list<pair<string,int> >::const_iterator end  = parameters.end();
	while (curr != end) {
          if ((*curr).first == name)
             return (*curr).second;
          curr++;
  ...
  ///
  const Parameter& find_parameter(const string& str) const
	{ return *(param_info[parameter_index(str)]); }

  ///
  Parameter& find_parameter(const string& str)
	{ return *(param_info[parameter_index(str)]); }

  ///
  vector<Parameter*> param_info;
  ...
  ///
  map<string, map<string,int>* > categorized_parameters;
#else
  ///
...
//
//
inline ParameterSet& ParameterSet::operator=(const ParameterSet& params)
{
vector<Parameter*>::const_iterator curr = params.param_info.begin();
vector<Parameter*>::const_iterator last = params.param_info.end();
while (curr != last) {
  add_parameter( **curr );
  curr++;
  }
...
//
template <class T>
void ParameterSet::set_parameter(const string& name, const T& value)
{
try {
...
//
template <class T>
void ParameterSet::get_parameter(const string& name, T& value) const
{
try {
...
//
template <class T>
T& ParameterSet::get_parameter(const string& name)
{
T* ptr = 0;
try {
  Parameter& param = find_parameter(name);
...
//
//
inline bool ParameterSet::parameter_initialized(const string& name)
{
try {
  Parameter& param = find_parameter(name);
...
{
vector<Parameter*>::iterator curr = param_info.begin();
vector<Parameter*>::iterator last = param_info.end();
while (curr != last) {
  if ( !((*curr)->disabled) )
...
//
//
inline void ParameterSet::disable_parameter(const string& name)
{
try {
  Parameter& param = find_parameter(name);
  param.disabled = true;
...
//
//
inline void ParameterSet::enable_parameter(const string& name)
{
try {
  Parameter& param = find_parameter(name);
...
//
template <class T>
void ParameterSet::create_categorized_parameter(const string& name, T& data, 
				const string& syntax,
...
//
template <>
inline void ParameterSet::create_categorized_parameter<char*>(const string& name, char*& , const string&, const string&, const string&, const string& )
{
EXCEPTION_MNGR(runtime_error,
...
//
//
inline void ParameterSet::set_parameter_default(const string& name, const string& value)
{
try {
  Parameter& param = find_parameter(name);
...
//
//
inline Parameter& ParameterSet::get_parameter_object(const string& name)
{
try {
...
#if defined(REDSTORM) || defined(COUGAR) || defined(TFLOPS)
  return find_parameter(name);
#endif
}
...
//
//
inline void ParameterSet::set_parameter_callback(const string& name, 
			std::unary_function<AnyReference,void>& func)
{
try {
  Parameter& param = find_parameter(name);
...
{
const string& tmp = any_cast<string>(value);
std::ifstream ifstr;
