...
//
map<string, map<string,int>* >::const_iterator curr = 
						categorized_parameters.begin();
map<string, map<string,int>* >::const_iterator end  = 
...
if (!verbose) {
   ...
   list<pair<string,int> >::const_iterator last = parameters.end();
   #else
   map<string,int>::const_iterator curr = parameters.begin();
   map<string,int>::const_iterator last = parameters.end();
   #endif
   while (curr != last) {
...
map<string, map<string,int>* >::iterator curr = categorized_parameters.find(tmp_category);
if (curr == categorized_parameters.end())
   categorized_parameters[tmp_category] = new map<string,int>;
(*categorized_parameters[tmp_category])[param->name] = tmp;

parameters[param->name] = tmp;
#else
parameters.push_front(pair<string,int>(param->name,tmp));
#endif
//cerr << param->name << " " << tmp << endl << flush;
...
if (param->aliases.size() > 0) {
   ...
   while (curr != end) {
     ...
     #else
     parameters.push_back(pair<string,int>((*curr),index));
     #endif
     curr++;
     }
   }
...
#else
  parameters.push_back(pair<string,int>(alias,index));
#endif
  }
catch (invalid_argument& ) {
  EXCEPTION_MNGR(runtime_error,
...
map<string, map<string,int>* >::iterator curr = pset.categorized_parameters.begin();
map<string, map<string,int>* >::iterator end  = pset.categorized_parameters.end();
while (curr != end) {
  map<string, int>::iterator lcurr = (*curr).second->begin();
  map<string, int>::iterator lend  = (*curr).second->end();
  while (lcurr != lend) {
...
vector<Parameter*>::iterator curr = pset.param_info.begin();
vector<Parameter*>::iterator end  = pset.param_info.end();
while (curr != end) {
  if (!pset.parameter_exists((*curr)->name)) {	
     Parameter* tmp = *curr;
     add_parameter(*tmp,"General");
     }
  curr++;
  }
#endif
...
vector<Parameter*>::iterator tcurr = pset.param_info.begin();
vector<Parameter*>::iterator tlast = pset.param_info.end();
while (tcurr != tlast) {
  if (overwrite || !parameter_exists((*tcurr)->name)) {
     Parameter* tmp = *tcurr;
     add_parameter(*tmp,false);
     }
  tcurr++;
  }
}
