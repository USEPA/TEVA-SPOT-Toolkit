...
public:
  ...
  typedef __reverse_bi_iterator<iterator,
      ...
      reverse_iterator;
#elif  defined(COUGAR) || defined(TFLOPS_SERVICE)
  ...
  iterator insert(iterator __position, const _Tp& __x)
	{
	return insert(&__x, __position._M_node);
	}

  /// Insert an empty value before the specified position
  iterator insert(iterator __position)
	{ return insert(0,__position._M_node); }
   ...
   */
  void add( _Tp& data, ListItem<_Tp>*& item)
	{ item = insert(&data); }
   ...
   */
  ListItem<_Tp>* insert( const _Tp* data=(_Tp*)0, 
				ListItem<_Tp>* item = (ListItem<_Tp>*)0);
  ...
  /// Write out the list to an output stream
  void write(ostream& os) const;

  /// Read in a list from an input stream
...
template <class _Tp, class _Alloc>
ListItem<_Tp>* LinkedList<_Tp,_Alloc>::insert( const _Tp* data, 
							ListItem<_Tp>* next)
{
...
template <class _Tp, class _Alloc>
ListItem<_Tp>* LinkedList<_Tp,_Alloc>::find( _Tp& data)
{
ListItem<_Tp>* curr = (first == last ? 0 : first);
...
template <class _Tp, class _Alloc>
void LinkedList<_Tp,_Alloc>::write(ostream& os) const
{
ListItem<_Tp>* curr = (first == last ? 0 : first);
...
template <class _Tp, class _Alloc>
void LinkedList<_Tp,_Alloc>::read(istream& is)
{
int tmp;
is >> tmp;
for (int i=0; i<tmp; i++) {
...
template <class _Tp, class _Alloc>
inline ostream& operator<<(ostream& os, const utilib::LinkedList<_Tp,_Alloc>& obj)
{ obj.write(os); return os; }

template <class _Tp, class _Alloc>
inline istream& operator>>(istream& is, utilib::LinkedList<_Tp,_Alloc>& obj)
