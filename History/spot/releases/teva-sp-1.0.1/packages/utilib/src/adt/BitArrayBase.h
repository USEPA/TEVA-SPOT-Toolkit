...
public:
  ...
  /// Copy constructor
  BitArrayBase<k,T,P>(BitArrayBase<k,T,P>& array) 
  ...
  virtual void put(const size_type ndx, const T val);
...
protected:
  ...
  void construct(const size_type             mylen,
                         bitword*            d=0,
                         const EnumDataOwned o=DataNotOwned)
	{ArrayBase<bitword,P>::construct(mylen,d,o); handle.base = this;}
  ...
  void dump_data(ostream& str, unsigned int max_elements=0);

  ///
  BitArrayBaseHandle<k,T,P> handle;
};
...
template <int k, class T, class P>
inline T BitArrayBase<k,T,P>::operator[](const size_type ndx) const
{
#if (ArraySanityChecking==1)
  if ((size_type)ndx >= Len)
    EXCEPTION_MNGR(runtime_error, "BitArrayBase::operator[] : iterator out of range. ndx=" << ndx << " len=" << Len)
#endif
  int shift = (ndx & index_mask) << k;
#ifdef RS6K
...
#if (ArraySanityChecking==1)
  if ((size_type)ndx >= Len)
     EXCEPTION_MNGR(runtime_error, "BitArrayBase::put : iterator out of range. ndx=" << ndx << " len=" << Len)
  if ((val < 0) || (val > static_cast<int>(data_mask)))
     EXCEPTION_MNGR(runtime_error, "BitArrayBase::put : value out of range. val=" << val << " ndx=" << ndx )
#endif
#ifdef RS6K
...
#endif
  int shift = (ndx & index_mask) << k;
  Data[i] = (Data[i] & ~(data_mask << shift)) | (val << shift);
}
...
template<int k, class T, class P>
int BitArrayBase<k,T,P>::read(istream& s)
{
  ...
  s >> reportedLen;
  if (reportedLen != Len)
    EXCEPTION_MNGR(runtime_error, "BitArray::read: size mismatch: " << Len << " (current) vs " << reportedLen << "(new)");

  s >> c;
  if (c != ':')
    EXCEPTION_MNGR(runtime_error, "BitArray::read: format error.  Expected : after length ");
...
int BitArrayBase<k,T,P>::translate_from_char(const char c) const
{
  ...
  EXCEPTION_MNGR(runtime_error, "BitArray::read : Bad input value: \'" << c << "\'")
  return 0;  // Never happens
}


template<int k, class T, class P>
int BitArrayBase<k,T,P>::write(ostream& output) const
{
  output << Len << ": ";
  if (Data) 
    {
...
template<int k, class T, class P>
void BitArrayBase<k,T,P>::dump_data(ostream& str, unsigned int max_elements)
{
  if (Data) 
    {
      bitword* pointer = Data;
...
template<int k, class T, class P>
inline
ostream& operator<<(ostream& output, const utilib::BitArrayBaseHandle<k,T,P>& array)
...
template<int k, class T, class P>
inline
istream& operator>>(istream& input, utilib::BitArrayBaseHandle<k,T,P>& array)
{
T tmp;
input >> tmp;
array = tmp;
...
template<int k, class T, class P>
inline
ostream& operator<<(ostream& output, const utilib::BitArrayBase<k,T,P>& array)
...
template<int k, class T, class P>
inline
istream& operator>>(istream& input, utilib::BitArrayBase<k,T,P>& array)
...
{
  if (x.size() != vec.size())
    EXCEPTION_MNGR(runtime_error, "BitArray operator<< : Unequal array lengths " << x.size() << " and " << vec.size())
  if (x.data() == vec.data())
    return x;
...
if ((value < 0) || ((static_cast<int>(value)) > (static_cast<int>(x.data_mask))))
   EXCEPTION_MNGR(runtime_error, "operator<<(const T value) : value " << value << " out of range for " << static_cast<int>(x.bits) << "-bit array")
#endif
...
unsigned int nwords = x.alloc_size(x.size());
