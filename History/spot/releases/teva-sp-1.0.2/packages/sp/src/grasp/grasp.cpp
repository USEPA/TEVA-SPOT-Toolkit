...
// TBD - change to return an elite solution (drop the input 's')
// TBD - have it also pass in an instance
		    ...
		    ) 
{
  ...
  for(int i=1;i<=numberOfIterations;i++) 
      ...
      else 
	{
      ...
      PMSearch::runLocalSearch(s,localSearchMethod);
      ...
      if(elitePool!=0) 
	{
	  ...
	  if(d>=1) 
		...
		if (!r->equals(s)) { //if they are not equal, try to relink them
		  ...
		  //--------------------------------
		  double newcost = PMRelink::combine (rs, s, r, recombinationMethod); //s is the one produced by local search, r is the elite one
		  if (verbose==true)
		    {
...
void Grasp::testLocalSearch(PMInstance *inst, PMConstructive *constr, char *method, double mt, bool output_density, bool profile) 
{
  ...
  while (1) {
    ...
    PMSearch::runLocalSearch(temp, method, output_density, profile ? times : 0); 
    t = timer.getTime();
    runs++;
    if (t > mt) break;
  }
...
int Grasp::run(int argc, char *argv[]) 
{
  ...
  if(strlen(configuration.inputsol)>0) 
    ...
    {
      PMConstructive::addRandom(s,instance->getP()); // starts with a random solution
    }
  ...
  if(configuration.run_grasp)
    { 
      grasp(s,&constructive,configuration.graspit,elite,configuration.ls,configuration.cmg,true,configuration.partials);
    }
  ...
  if((elite!=0)&&(configuration.test_pr==false)&&(configuration.postopt==true)) 
    {
      ...
      PMRelink::pathRelinking(s, elite, configuration.cmp, true);
    } 
  ...
  std::cout << std::endl;
  std::cout << "Number of attacks " << instance->getN() << std::endl;
...
{
  std::set_new_handler(outOfMemoryTrap);
