 ...
 * \if GeneratingHTML [\ref SleTar85 "SleTar85"]. \endif
...
while (tree) {
  remove(tree,status);
  if (!status)
     EXCEPTION_MNGR(runtime_error, "AbstractSplayTree<T,KEY>::clear - problem removing the root of the splay tree.")
  }
}
...
/// Out-stream operator for AbstractSplayTree objects
template <class T, class KEY>
inline ostream& operator<<(ostream& output, utilib::AbstractSplayTree<T,KEY>& tree)
{ tree.write(output); return(output); }


/// In-stream operator for AbstractSplayTree objects
template <class T, class KEY>
inline istream& operator>>(istream& input,
						utilib::AbstractSplayTree<T,KEY>& tree)
