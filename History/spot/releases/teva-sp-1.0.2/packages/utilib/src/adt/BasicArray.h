...
public:
  ...
  ///Makes reference be a & of the datatype in the BasicArray
  typedef const typename iterator_traits<_Iterator>::reference const_reference;
 ...
 */
template <class T>
class BasicArray : public ArrayBase< T,BasicArray<T> >
{
public:
...
//
template <class T>
const T & BasicArray<T>::operator[](const size_type idx) const
{
#if (ArraySanityChecking==1)
#if defined(REDSTORM) || defined(COUGAR) || defined(TFLOPS)
if ((size_type)idx >= Len)
				       ...
				       const size_type newlen)
{
  ...
  if (array.Data)
    set_data(&(array.Data[start]), newlen, DataNotOwned);
  else
    base_t::resize(0);
...
vector<TYPE>& operator<< ( vector<TYPE>& vec, const utilib::BasicArray<BTYPE>& old)
{
vec.resize(old.size());
...
///
template <class TYPE, class BTYPE>
utilib::BasicArray<BTYPE>& operator<< ( utilib::BasicArray<BTYPE>& vec, const vector<TYPE>& old)
{
vec.resize(old.size());
typename vector<TYPE>::const_iterator curr = old.begin();
typename vector<TYPE>::const_iterator last = old.end();
...
/// BasicArray out-stream operator
template <class T>
ostream& operator<<(ostream& os, const utilib::BasicArray<T>& array)
{
size_type mylen = array.size();
os << mylen << " : ";
T* tmp = array.data();
...
/// BasicArray in-stream operator
template <class T>
istream& operator>>(istream& is, utilib::BasicArray<T>& array)
{
size_type len;
is >> len;
array.resize(len);
...
/// BasicArray PackBuffer operator
template <class T>
utilib::PackBuffer& operator<<(utilib::PackBuffer& os, const utilib::BasicArray<T>& array)
{
size_type mylen = array.size();
os << mylen;
T* tmp = array.data();
...
/// BasicArray UnPackBuffer operator
template <class T>
utilib::UnPackBuffer& operator>>(utilib::UnPackBuffer& is, utilib::BasicArray<T>& array)
{
size_type len=0;
is >> len;
array.resize(len);
