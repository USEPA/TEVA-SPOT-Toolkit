	...
	}
	{
		...
		if(hasData(simResultsRef,"demands",FLOAT_ARRAY_2D_TYPE,&hasDemand)==ND_FAILURE) {
			...
			if(hasDemand) {
				...
				for(j=0;j<nnodes;j++) {
					if(i==0) { node[j].info->q = (float *)calloc(nsteps,sizeof(float)); }
			...
			if(hasFlow) {
				...
				for(j=0;j<nlinks;j++) {
					if(i==0) { link[j].info->flow = (float *)calloc(nsteps,sizeof(float)); }
					link[j].info->flow[i] = tres[j];
				}
				nd_free(tres);
			}
			if(hasVel) {
				...
				for(j=0;j<nlinks;j++) {
					if(i==0) { link[j].info->velocity = (float *)calloc(nsteps,sizeof(float)); }
					link[j].info->velocity[i] = tres[j];
				}
				nd_free(tres);
			}
		}
...
JNIEXPORT PDoseOverThresh allocateDoseOverThresholdWithIDs(void *analysisOptionsRef, PMem mem) {
  ...
  nnodes=mem->net->info->nnodes;
  for(i=0;i<numThresh;i++) {
    pdot->numOver[i] = (int *)calloc(mem->net->info->nnodes,sizeof(int));
    pdot->numOverByTime[i] = (int *)calloc(mem->net->info->nsteps,sizeof(int));
  }
  pdot->totOver = (int *)calloc(numThresh,sizeof(int));
  return pdot;
