...
**--------------------------------------------------------------
*/
void DLLEXPORT TSO_ReadPrologue(PTSO tso, PNetInfo *netInfo, PNodeInfo *nodes,PLinkInfo *links, int flags)
{
    int i,j,status=0;
	char msg[256];
    FILE *TSO;
...
static int x_TSO_ReadSimulationResults(FILE *fpTSO, PTSO tso, PNetInfo net, PNodeInfo nodes,PSource sources)
{
	...
	for(s=0;s<nbulkspecies;s++) {
		if(tso->storageMethod==0) {
			...
			for(i=0;i<nsteps;i++) {
				int j;
				if((nread=fread(tso->nodedata,sizeof(float),nnodes,fpTSO)) < (unsigned int)nnodes) {
					status=1;
			...
			for(i=0;i<nnz;i++) {
				...
				if(tso->storageMethod==1) {
					...
					if((nread=fread(nodes[idx].c[s],sizeof(float),nsteps,fpTSO)) < (unsigned int)nsteps) {
						status=1;
					...
					for(i=0;i<numGroups;i++) {
						...
						max += gh.count;
						if (gh.groupType==1) {
							if((nread=fread(&c[n],sizeof(float),gh.count,fpTSO)) < (unsigned int)gh.count) {
								status=1;
...
int DLLEXPORT TSO_WriteSimResults(PTSO tso, PNetInfo net,PSource sources,PNodeInfo nodes)
{
	...
	if(tso->fileVersion>=8) {
		if(!x_writeIndexInfo(tso->indexStream, tso->count,fpos,0,bw)) return 0;
		tso->count++;
...
void DLLEXPORT TSO_ReleaseNetworkData(PNodeInfo *nodes, PLinkInfo *links, PNetInfo net) {
    ...
    for (i=0; i<net->nnodes; i++) {                                        /* Tank indices */
		...
		if(tnodes[i].c != NULL) free(tnodes[i].c);
        if(tnodes[i].q != NULL) free(tnodes[i].q);
	...
	*nodes=NULL;
    ...
    for (i=0; i<net->nlinks; i++) {                                        /* From link nodes */
		if(tlinks[i].flow != NULL) free(tlinks[i].flow);
		if(tlinks[i].velocity != NULL) free(tlinks[i].velocity);
        free(tlinks[i].id);
        free(tlinks[i].vx);
        free(tlinks[i].vy);
    }
