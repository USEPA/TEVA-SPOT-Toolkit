...
Adding \doxyref{DVF File}{p.}{formats_dvfFile} for flushing control allows the user to simulate a flushing response to an event. Tevasim behaves slightly differently in this case, adding in the flushing rate to demands at the specified nodes, and closing the specified pipes. The number of pipes and/or nodes can be zero, if you don't want to use that part of the response policy.

\section{Using tso2Impact}\label{impacts_impact2}

After running \doxyref{tevasim}{p.}{tevasimExecutable} command, the output files, {\ttfamily Net3.tso} and {\ttfamily Net3.sdx}, can be used to compute one or more IMPACT files. An IMPACT file summarizes the consequence of each contamination incident in a manner that facilitates optimization. The \doxyref{tso2Impact}{p.}{tso2ImpactExecutable} command generates these files with the following command line: 
