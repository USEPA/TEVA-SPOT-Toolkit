...
def run():
    ...
    if not os.path.exists('python'):
        if trunk:
            ...
            rc = subprocess.call(['bin/install_python']+site_packages+['--trunk','--logfile','tpl/python.log'])
        else:
            print "Installing Python from cached packages"
            rc = subprocess.call(['bin/install_python']+site_packages+['--logfile','tpl/python.log'])
    if rc < 0:
        return
