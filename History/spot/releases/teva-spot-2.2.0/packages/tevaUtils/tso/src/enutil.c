...
static void ENTUExpandGen(int, int, int, float, long, long, int, int, char**, PSource, FILE*);
...
void ENTU_GetLinkHData(PLinkInfo links, PNetInfo net, int time, long tstep)
/*
**-----------------------------------------------------------
...
void  ENTU_GetNodeHData(PNodeInfo nodes, PNetInfo net, int time, long tstep)
/*
**-----------------------------------------------------------
...
void  ENTU_GetNodeQData(PNodeInfo nodes, PNetInfo net, int time, long tstep)
/*
**-----------------------------------------------------------
**  Input:   TSO network and node structures, time step
**  Output:  None
...
void  ENTU_GetLinkQData(PLinkInfo links, PNetInfo net, int time, long tstep)
/*
**-----------------------------------------------------------
...
void  ENTU_ClearNodeQData(PNodeInfo nodes, PNetInfo net)
/*
**-----------------------------------------------------------
...
void  ENTU_GetNodeXY(PNodeInfo nodes, PLinkInfo links, PNetInfo net, char *epanetfname)
/*
**--------------------------------------------------------------
...
void ENTU_EpanetError(int errnum)
/*
**-----------------------------------------------------------
...
void  ENTU_WriteTSI(PNetInfo net, PSource sources, FILE *simgen, FILE *simin)
/*
**--------------------------------------------------------------
...
*/
{
    ...
    while (fgets(line,MAXLINE,simgen) != NULL)
    {
        ...
        ENTUExpandGen(nnodes, sourcetype, speciesindex, sourcestrength, starttime, stoptime, numsources, 0, tok, sources, simin);
...
static void ENTUExpandGen(int nnodes, int sourcetype, int speciesindex, float sourcestrength, long starttime, 
        ...
        if ( ENTUmatch(nodetok[is],"ALL") ) {
            for (i=1; i<=nnodes; i++) {
                ...
                if (nodetype == EN_JUNCTION) {
                    ENTUExpandGen(nnodes, sourcetype, speciesindex, sourcestrength, starttime, stoptime, numsources, is+1, nodetok, sources, simin);
                }
            }
        }
...
int ENTU_SetSource(PSource sources, PNetInfo net, FILE *simin)
/*
**--------------------------------------------------------------
...
*/
{
    ...
    while (fgets(line,MAXLINE,simin) != NULL)
    {
        ...
        for (is = 0; is < net->nsources; is++) {
            ...
            if ( sources[is].SpeciesIndex < 1 || sources[is].SpeciesIndex > net->nspecies ) enutilerror(1,"TSI file error");
...
void ENTU_Initialize(char *epanetfname, char *tsofname, int storageMethod, int tsofileversion, PTSO *tso, PNetInfo *net, PNodeInfo *nodes, PLinkInfo *links, PSource *sources) {
    ...
    TSO_AllocateNetworkData(nodes,links,tnet);
...
//printf("ENTU_CreateDemandPatterns: 01\n");
			ENgetbasedemand(enIdx,d,&demands[d]);
//printf("ENTU_CreateDemandPatterns: 04\n");
			if(combinedLength==0) combinedLength=allPatts[d]->numValues;
			else combinedLength=lcm(combinedLength,allPatts[d]->numValues);
//printf("ENTU_CreateDemandPatterns: 05\n");
		}
		float *combinedDemands=(float*)calloc(combinedLength,sizeof(float));
//printf("ENTU_CreateDemandPatterns: 08\n");
		for(d=0;d<numDemands;d++) {
			float baseDemand=demands[d];
			PatternInfoPtr patt=allPatts[d];
			float*mult=patt->mult;
			int i=0;
//printf("ENTU_CreateDemandPatterns: 10\n");
...
//printf("ENTU_CreateDemandPatterns: 15\n");
		}
		node->demandProfile=combinedDemands;
		node->demandProfileLength=combinedLength;
//printf("ENTU_CreateDemandPatterns: 20\n");
		fprintf(fp,"%s\t%d",node->id,node->demandProfileLength);
		for(d=0;d<node->demandProfileLength;d++) {
			fprintf(fp,"\t%f",node->demandProfile[d]);
		}
		fprintf(fp,"\n");
		free(allPatts);
		free(demands);
