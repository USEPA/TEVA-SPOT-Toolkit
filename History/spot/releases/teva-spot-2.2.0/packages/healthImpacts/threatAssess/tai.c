...
//					}
        ...
        case _DOSE_THRESHOLDS: {
				...
				PDoseOverThresh pdot = (PDoseOverThresh)calloc(1,sizeof(DoseOverThresh));
				...
				for(j=0;j<pdot->numThresh;j++) {
					...
					pdot->numOver[j] = (int *)calloc(mem->net->info->nnodes,sizeof(int));
					pdot->numOverByTime[j] = (int *)calloc(mem->net->info->nsteps,sizeof(int));
				}
				mem->dot=pdot;
			}
			break;
...
//            strcpy(io->tsoname,tok[1]);
    ...
    initTA(mem->ta,mem->net->info->nnodes);                  /* Temporal averages */
