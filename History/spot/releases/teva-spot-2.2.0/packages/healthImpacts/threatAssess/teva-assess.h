...
void Population(PNet net,PopulationDataPtr pdp,PNode node);
void initializeIngestionData(IngestionDataPtr p, PNet net, PNode nodes, PDR dr, PopulationDataPtr pd);
void  IngestionRate(PNet,PNode);
void  ResponseOld(PNet,PDR,PU);
void Response(IndividualIngestionDataPtr *idp, IngestionDataPtr ingData, PDR dr,PNode node, PU u);

void  DoseOld(int,PNet,PDR,PU,PNode,float);
void Dose(IndividualIngestionDataPtr *idp, IngestionDataPtr ingData, PNode node, PNet net, int detectTime, PU u, PDoseOverThresh dot, int nodeidx, PDoseBinData doseBins);

float DoseResponse(float,PDR);
float ResponseDose(float response, PDR dr);
...
void dumpHIAInputParameters(PMem mem);

/** NEW **/
void resetDoseOverThreshold(PMem mem);
void resetDoseBinData(PMem mem);
void initXA(PXA xa, int len);
void resetXA(PXA xa, int len);
void freeXA(PXA xa, int len);
void freeHistogramData(PHIST hist);
void initTA(PTA ta, int len);
void resetTA(PTA ta, int len);
...
void freeDoseOverThreshold(PMem mem, PDoseOverThresh *dot);
/** End NEW **/
