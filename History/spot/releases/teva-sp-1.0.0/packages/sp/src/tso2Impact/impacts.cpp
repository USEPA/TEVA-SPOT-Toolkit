...
void read_impacts(std::istream& istr, utilib::BasicArray<std::map<int,double> >& impacts, int delay)
{
...
void write_impact_statistics(int id, const utilib::BasicArray<int>& sensors,
			...
			utilib::BasicArray<utilib::BasicArray<std::map<int,double> > >& impacts,
...
if (format == "cout") {
   ...
   utilib::BasicArray<utilib::BasicArray<std::map<int,double> > >::iterator icurr = impacts.begin();
   ...
   for (; fcurr != fend; fcurr++, curr++, icurr++) {
     ...
     std::cout << "Value at Risk (VaR):        " << stat.VaR << std::endl;
     std::cout << "TCE:                        " << stat.TCE << std::endl;
