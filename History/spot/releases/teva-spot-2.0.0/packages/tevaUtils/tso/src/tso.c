...
static void x_TSO_LoadSources(FILE *fpTSO, PTSO tso, PNetInfo net, PNodeInfo nodes,PSource sources);
static void x_TSO_positionFile(FILE *fp,__file_pos_t offs);
...
static void x_TSO_ReadSizes(PTSO tso, PNetInfo *net)
{
    ...
    long simduration,reportstart,reportstep;
...
int TSO_GetSimulationResults(int id, PTSO tso, PNetInfo net, PNodeInfo nodes,PSource sources)
{
	...
	x_TSO_positionFile(tso->streams[di->fileIndex],di->fileOffset);
	return x_TSO_ReadSimulationResults(tso->streams[di->fileIndex],tso,net,nodes,sources);
}

void x_TSO_positionFile(FILE *fp,__file_pos_t offs)
{
    fpos_t pos;
...
int TSO_GetRawSimulationResults(int id, PTSO tso, PNetInfo net, PNodeInfo nodes,PSource sources, char *buf, int len)
{
	...
	x_TSO_positionFile(tso->streams[di->fileIndex],di->fileOffset);
	x_TSO_LoadSources(tso->streams[di->fileIndex],tso,net,nodes,sources);
...
static void x_TSO_LoadSources(FILE *fpTSO, PTSO tso, PNetInfo net, PNodeInfo nodes,PSource sources) {
	if(tso->fileVersion >= 8) {
		...
		x_TSO_ReadSources(fpTSO,&net->nsources,&sources,nodes,tso->fileVersion);
	}
...
static int x_TSO_ReadSimulationResults(FILE *fpTSO, PTSO tso, PNetInfo net, PNodeInfo nodes,PSource sources)
{
	...
	x_TSO_LoadSources(fpTSO,tso,net,nodes,sources);
