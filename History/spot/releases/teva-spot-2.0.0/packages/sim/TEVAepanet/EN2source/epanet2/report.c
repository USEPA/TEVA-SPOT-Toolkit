...
*/
{
   ...
   writeline(s);
   sprintf(s,FMT25,RptFormTxt[(size_t)Formflag]);
   writeline(s);
...
/*** Updated 6/24/02 ***/
   ...
   */
   for (i=1; i<=Ntanks; i++)
   {
      ...
      {
         if (Tank[i].A > 0.0)
            sprintf(s1,FMT50,atime,Node[n].ID,StatTxt[(size_t)newstat],
               (H[n]-Node[n].El)*Ucf[HEAD],Field[HEAD].Units);
         else sprintf(s1,FMT51,atime,Node[n].ID,StatTxt[(size_t)newstat]);
         writeline(s1);
   ...
   for (i=1; i<=Nlinks; i++)
   {
      ...
      {
         if (Htime == 0)
            sprintf(s1,FMT52,atime,LinkTxt[(size_t)Link[i].Type],Link[i].ID,
               StatTxt[(size_t)S[i]]);
         else sprintf(s1,FMT53,atime,LinkTxt[(size_t)Link[i].Type],Link[i].ID,
            StatTxt[(size_t)OldStat[i]],StatTxt[(size_t)S[i]]);
         writeline(s1);
         OldStat[i] = S[i];
      }
   }
...
*/
{
   ...
   if (type == NODEHDR)
   {
      ...
      else if (Tstatflag != SERIES) sprintf(s,FMT76,TstatTxt[(size_t)Tstatflag]);
   ...
   if (type == LINKHDR)
   {
      ...
      else if (Tstatflag != SERIES) sprintf(s,FMT79,TstatTxt[(size_t)Tstatflag]);
...
*/
{
   ...
   if (Rptflag)
   {
      ...
      {
         PageNum++;
         if (fprintf(RptFile,FMT82,PageNum,Title[(size_t)0]) == EOF)
            Fprinterr = TRUE;
         LineNum = 3;
      }
   }
...
/*** Updated 10/25/00 ***/
      ...
      sprintf(Msg,FMT56,LinkTxt[(size_t)Link[k].Type],Link[k].ID,setting);
      writeline(Msg);
      return;
   }
...
/* We have a status change. Write the old & new status types. */
   ...
   if (j1 != j2)
   {
      sprintf(Msg,FMT57,LinkTxt[(size_t)Link[k].Type],Link[k].ID,
...
*/
{
   ...
   switch (Control[i].Type)
   {
      ...
      case HILEVEL:
         n = Control[i].Node;
         sprintf(Msg,FMT54,clocktime(Atime,Htime),LinkTxt[(size_t)Link[k].Type],
            Link[k].ID,NodeTxt[getnodetype(n)],Node[n].ID);
         break;
      case TIMER:
      case TIMEOFDAY:
         sprintf(Msg,FMT55,clocktime(Atime,Htime),LinkTxt[(size_t)Link[k].Type],
            Link[k].ID);
         break;
      default: return;
   }
...
*/
{
   sprintf(Msg,FMT63,clocktime(Atime,Htime),LinkTxt[(size_t)Link[k].Type],
      Link[k].ID,ruleID);
   writeline(Msg);
}
...
*/
{
   ...
   for (i=1; i<=Nvalves; i++)
   {
      ...
      if (S[j] >= XFCV)
      {
         sprintf(Msg,WARN05,LinkTxt[(size_t)Link[j].Type],Link[j].ID,
            StatTxt[(size_t)S[j]],clocktime(Atime,Htime));
         if (Messageflag) writeline(Msg);
         flag = 5;
      }
   }
   ...
   for (i=1; i<=Npumps; i++)
   {
      ...
      {
         sprintf(Msg,WARN04,Link[j].ID,StatTxt[(size_t)S[j]],
