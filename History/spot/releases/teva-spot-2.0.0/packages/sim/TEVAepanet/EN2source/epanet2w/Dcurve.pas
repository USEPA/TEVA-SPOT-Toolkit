...
// Draw curve & data points (except for fitted pump curve)
  with Series1 do
  begin
    Active := False;
    for I := 1 to N do
      AddXY(X[I], Y[I], '', clTeeColor);
    if IsPumpCurve then Pointer.Visible := False
    else Pointer.Visible := True;
    Active := True;
  end;

// Display original pump curve data points on fitted pump curve
  if IsPumpCurve then with Series2 do
  begin
    Active := False;
    N := GetData;
    for I := 1 to N do
