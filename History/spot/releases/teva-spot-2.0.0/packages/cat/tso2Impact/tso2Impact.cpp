...

{
      ...
      if(pkSelected==true || peSelected==true || pdSelected==true || dpkSelected || dpeSelected || dpdSelected) 
	{
	  ...
	  initializeIngestionData(assessMem->ingestionData,assessMem->net,assessMem->node,0.5f,assessMem->dr,assessMem->popData);
...
usage:\n\
\n\
   tso2Impact <output-prefix> <tso-directory-or-files> [options...]\n\
\n\
options:\n\
\n\
   -responseTime<minutes>\n\
	This parameter indicates that the detection impact will be\n\
	computed some minutes after detection due to a response delay.\n\
        This option was formerly called detectionDelay.\n\
\n\
   -detectionConfidence <number of sensors>\n\
        The number of sensors that must detect an event before the    \n\
        impacts are calculated.  Normally this is 1 sensor.           \n\
\n\
   -minQuality <threshold,threshold, ..., threshold>\n\
	This parameter indicates the minimum threshold needed to perform\n\
	detection with a sensor.  There must be one threshold for each \n\
        .tso file.\n\
\n\
   -mc\n\
...
\n\
   -dmc\n\
	If this flag is specified, an impact file wll be generated for the\n\
	'detected mass consumed' measure.\n\
\n\
   -vc\n\
	If this flag is specified, an impact file wll be generated for the\n\
...
\n\
   -dvc\n\
	If this flag is specified, an impact file wll be generated for the\n\
	'detected volume consumed' measure.\n\
\n\
   -td\n\
	If this flag is specified, an impact file wll be generated for the\n\
...
\n\
   -dtd\n\
	If this flag is specified, an impact file wll be generated for the\n\
	'detected time-to-detection' measure.\n\
\n\
   -nfd\n\
	If this flag is specified, an impact file wll be generated for the\n\
...
\n\
   -dec\n\
	If this flag is specified, an impact file wll be generated for the\n\
	'detected extent of contamination' measure.\n\
" << std::endl;
#if TEVA_SPOT_LIMITED
	std::cerr << "\
\n\
   -pk <tai-input-file>\n\
	...
	The number of TAI files must match the number of TSO files. \n\
\n\
   -dpk <tai-input-file>\n\
	If this parameter is specified, an impact file wll be generated for the\n\
	'detected population killed' measure.  The TAI input file is a threat_assess\n\
	input that specifies parameters like dosage, response, lethality, etc\n\
	Note: it is an intensive measure to compute.\n\
	The number of TAI files must match the number of TSO files. \n\
\n\
...
\n\
   -dpd <tai-input-file>\n\
	If this parameter is specified, an impact file wll be generated for the\n\
	'detected population dosed' measure.  The TAI input file is a threat_assess\n\
	input that specifies parameters like dosage, response, lethality, etc\n\
	Note: it is an intensive measure to compute.\n\
	The number of TAI files must match the number of TSO files. \n\
\n\
...
\n\
   -dpe <tai-input-files>\n\
	If this parameter is specified, an impact file wll be generated for the\n\
	'detected population exposed' measure.  The TAI input file is a threat_assess\n\
	input that specifies parameters like dosage, response, lethality, etc\n\
	This measure concerns the number of people 'sickened' by exposure.\n\
	The number of TAI files must match the number of TSO files. \n\
" << std::endl;
...
" << std::endl;
      ...
      while ((currentArgIndex < argc) && (argv[currentArgIndex][0] != '-')) {
        tsoFileOrDirectoryName.push_back(argv[currentArgIndex]);
	currentArgIndex++;
        }
      ...
      while(currentArgIndex<argc)
	{
	  if(strcmp(argv[currentArgIndex],"-mc")==0) 
	  ...
	  else if(strcmp(argv[currentArgIndex],"-dmc")==0) 
	    {
	      dmcSelected=true;
	  ...
	  else if(strcmp(argv[currentArgIndex],"-dvc")==0) 
	    {
	      dvcSelected=true;
	  ...
	  else if(strcmp(argv[currentArgIndex],"-dtd")==0) 
	    {
	      dtdSelected=true;
	  ...
	  else if(strcmp(argv[currentArgIndex],"-dec")==0) 
	    {
	      decSelected=true;
	      ...
	      if(currentArgIndex==(argc-1))
		{
	      ...
	      currentArgIndex++;
      	      while (currentArgIndex < argc && argv[currentArgIndex][0] != '-'){
        	taiInputFilename.push_back(argv[currentArgIndex]);
		currentArgIndex++;
              }
	  ...
	  else if(strcmp(argv[currentArgIndex],"-dpk")==0) 
	    {
	      dpkSelected=true;
	      if(currentArgIndex==(argc-1))
		{
		  std::cerr << "***TAI input file not supplied - required for computation of detected population killed objective" << std::endl;
		  exit(1);
		}
	      currentArgIndex++;
      	      while (currentArgIndex < argc && argv[currentArgIndex][0] != '-'){
	  ...
	  else if(strcmp(argv[currentArgIndex],"-dpd")==0) 
	    {
	      dpdSelected=true;
	      if(currentArgIndex==(argc-1))
		{
		  std::cerr << "***TAI input file not supplied - required for computation of detected population dosed objective" << std::endl;
		  exit(1);
		}
	      currentArgIndex++;
      	      while (currentArgIndex < argc && argv[currentArgIndex][0] != '-'){
	  ...
	  else if(strcmp(argv[currentArgIndex],"-dpe")==0) 
	    {
	      dpeSelected=true;
	      if(currentArgIndex==(argc-1))
		{
		  std::cerr << "***TAI input file not supplied - required for computation of detected population exposed objective" << std::endl;
		  exit(1);
		}
	      currentArgIndex++;
      	      while (currentArgIndex < argc && argv[currentArgIndex][0] != '-'){
	  ...
	  else if((strcmp(argv[currentArgIndex],"-detectionConfidence")==0))
	    {
	      detectionConfidence=atoi(argv[++currentArgIndex]);
	      if(detectionConfidence<1)
		{
		  std::cerr << "***The detection confidence must be at least 1" << std::endl;
		  exit(1);
		}
	      std::cout << "Detection confidence=" << detectionConfidence << std::endl;
	    } 
	  else if((strcmp(argv[currentArgIndex],"-responseTime")==0) ||
	          (strcmp(argv[currentArgIndex],"-detectionDelay")==0) )
	    {
	...
	PTSO& tso)
{
      ...
      if(tsoPattern=="")
	{
std::cout << "OPEN NO PATTERN" << std::endl;
	  tso=TSO_Open((char*)tsoFileOrDirectoryName.c_str(),0);
	}
      else 
	{
std::cout << "OPEN YES PATTERN" << std::endl;
