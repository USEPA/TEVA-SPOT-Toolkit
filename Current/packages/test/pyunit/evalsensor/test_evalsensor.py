...
import pyutilib.th
from pyspot import pyunit, parse_sp
...
class Test(pyutilib.th.TestCase):
    ...
    def execute_test(self, options, sensor_file, network, impacts, name=None, nodemap=None):
        ...
        self.assertFileEqualsBaseline(currdir+name+"_parsed.out",currdir+name+".qa")
    ...
    def execute_other_test(self, options, sensor_file, network, impacts, name=None, nodemap=None):
        ...
        if name is None:
            name = network
        pyutilib.subprocess.run(cmd, currdir+name+".out",cwd=currdir)
        self.assertFileEqualsBaseline(currdir+name+".out",currdir+name+".qa")
        #os.remove(currdir+name+".out")
