...
/* 	float**   nodedata;  */
	...
	if(getInt(simResultsRef, "simStart", &simStart)==ND_FAILURE) return NULL;
	if(getFloat(simResultsRef, "stepSize", &stepsize)==ND_FAILURE) return NULL;
	if(getInt(simResultsRef, "numSpecies", &nspecies)==ND_FAILURE) return NULL;
	...
	mem->net = net;
/*	net->fltmax=fltmax; */
	net->info->numJunctions=njunctions;
	net->info->numLinks=nlinks;
	net->info->numNodes=nnodes;
	net->info->numSteps=nsteps;
	net->info->stepSize=stepsize;
	net->info->simStart=simStart;
	net->info->numSpecies=nspecies;
	net->info->qualResults=(PQualData)calloc(1,sizeof(QualData));
	net->info->qualResults->nodeC=(float ***)calloc(nspecies,sizeof(float **));
	net->info->qualResults->linkC=(float ***)calloc(nspecies,sizeof(float **));
	net->info->qualResults->nodeC[speciesIndex]=(float **)calloc(nnodes,sizeof(float*));
	for(i=0;i<nnodes;i++) {
		net->info->qualResults->nodeC[speciesIndex][i]=(float *)calloc(net->info->numSteps,sizeof(float));
	}
//	net->info->qualResults->linkC[0]=(float **)calloc(nlinks,sizeof(float*));
//	for(i=0;i<nlinks;i++) {
//		net->info->qualResults->linkC[0][i]=(float *)calloc(net->info->numSteps,sizeof(float));
//	}
	net->info->hydResults = (PHydData)calloc(1,sizeof(HydData));
	...
	for(i=0;i<nnodes;i++) {
		...
		node[i].info->nz = (int *)calloc(mem->net->info->numSpecies,sizeof(int));
	}
	...
	}
	{
		...
		if(hasDemand) {
			float **q;
			int j;
			q=net->info->hydResults->demand=(float **)calloc(net->info->numSteps,sizeof(float *));
		...
		if(hasFlow) {
			float **f;
			int j;
			f=net->info->hydResults->flow=(float **)calloc(net->info->numSteps,sizeof(float *));
			for(j=0;j<net->info->numSteps;j++) {
		...
		if(hasVel) {
			float **v;
			int j;
			v=net->info->hydResults->velocity=(float **)calloc(net->info->numSteps,sizeof(float *));
			for(j=0;j<net->info->numSteps;j++) {
		...
		for(i=0;i<nsteps;i++) {
			...
			if(hasFlow) {
				float **f=net->info->hydResults->flow;
			...
			if(hasVel) {
				float **v=net->info->hydResults->velocity;
	...
	}
	{
		...
		for(i=0;i<nnodes;i++) {
			...
			if(lens[i]>0) {
				...
				node[i].info->demandProfile=(float*)malloc(lens[i]*sizeof(float));
	...
	net->info->numTanks=ntanks;
	if(getIntArray(simResultsRef, "tankIndices", &tankIndices)==ND_FAILURE) return NULL;
	for(i=0;i<ntanks;i++)
		node[tankIndices[i]].info->type = tank;
	nd_free(tankIndices);

	if(getStringArray(simResultsRef, "nodeIDs", &nodeIDs)==ND_FAILURE) return NULL;
	for(i=0;i<nnodes;i++) {
		if (node[i].info->type != tank) node[i].info->type=junction;
		node[i].pop=0;
		strcpy(node[i].info->id,nodeIDs[i]);
	}
	nd_freeStringArray(nodeIDs,nnodes);
...
//ANL_UTIL_LogInfo(env,"teva.analysis.server","freeHIAMemory: 01");
	...
	for(i=0;i<nnodes;i++) {
		...
		if(mem->node[i].info->demandProfileLength>0) {
			free(mem->node[i].info->demandProfile);
		}
	}
	...
	free(mem->popData);
	if(mem->dr->speciesName != NULL)
		free(mem->dr->speciesName);
	free(mem->dr);

	{
		int nsteps=mem->net->info->numSteps;
		int nlinks=mem->net->info->numLinks;
		PQualData qd = mem->net->info->qualResults;
		PHydData hd = mem->net->info->hydResults;
		for(i=0;i<mem->net->info->numSpecies;i++) {
			int n;
			for(n=0;n<nnodes;n++) {
		...
		free(mem->net->info->qualResults);
		mem->net->info->qualResults=NULL;
		for(i=0;i<nsteps;i++) {
		...
		free(mem->net->info->hydResults);
		mem->net->info->hydResults=NULL;
	}
	free(mem->net->info);
	free(mem->net);
